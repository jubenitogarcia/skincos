...
export async function writeEmergencyLatch({
  env = process.env,
  fetchImpl = fetch,
  brokerPolicy,
  now,
  ...
  const reportFile = required(env, "PONTO_EMERGENCY_LATCH_REPORT");
  ...
  const report = {
    ...
    coordinatorRunId,
    emergencyRunId: triggerRunId,
    changedAt: payload.latch.changedAt,
    latched: true,
    observations,
...
if (invokedPath === import.meta.url) {
  await writeEmergencyLatch();
  process.stdout.write("Ponto emergency latch is fail-closed and independently attested.\n");
}
