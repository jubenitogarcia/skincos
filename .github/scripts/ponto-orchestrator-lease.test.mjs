...
test("staging Pages incumbent capture retries and requires exact terminal provenance", () => {
  ...
  assert.match(block, /deployments\?env=production&page=1&per_page=25/);
  assert.match(block, /item\?\.environment === "production" \|\| item\?\.environment == null/);
  assert.match(block, /item\?\.project_name === expectedProject/);
  assert.match(block, /item\?\.deployment_trigger\?\.metadata\?\.branch === "staging"/);
  assert.match(block, /item\?\.latest_stage\?\.status === "success"/);
  assert.match(block, /Number\.isFinite\(Date\.parse\(String\(item\?\.latest_stage\?\.ended_on/);
