...
nohup python "$ROOT_DIR/run_ui.py" --host=0.0.0.0 --port="${PORT}" > "$ROOT_DIR/ui_embed.out" 2>&1 &
PID=$!
...
echo "[restart] Últimas linhas de log:"; tail -n 25 "$ROOT_DIR/ui_embed.out" || true
