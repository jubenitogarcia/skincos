...
if [[ -x "$CRM_LOCAL_SCRIPT" ]]; then
  echo "[restart_crm] Aviso: usando o script canônico do CRM em $CRM_LOCAL_SCRIPT"
...
kill_procs() {
  ...
  pgrep -fl "api/server.js" | grep -q "crm" && pkill -f "api/server.js" 2>/dev/null || true
}
...
start_api() {
  if [[ $START_API -eq 1 ]]; then
    ...
    if [[ $WATCH_MODE -eq 1 ]]; then
      (cd "$CRM_DIR" && npx nodemon --quiet --watch api --ext js,mjs,cjs,json api/server.js >"$API_LOG" 2>&1 &)
      API_PID=$!
    else
      node "$CRM_DIR/api/server.js" >"$API_LOG" 2>&1 &
      API_PID=$!
    fi
    sleep 2
