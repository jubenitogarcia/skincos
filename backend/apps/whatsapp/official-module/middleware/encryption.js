...
function getMasterKey() {
    // Resolve to monorepo `backend/` folder (not the repo root) so the default
    // VAR dir becomes `backend/var/` when running this module standalone.
    const backendRoot = path.resolve(__dirname, '../../../..');
    const defaultRepoFile = path.join(__dirname, '..', '.master-key');
    const varDir = process.env.VAR_DIR || path.join(backendRoot, 'var');
    const defaultVarFile = path.join(varDir, 'whatsapp', 'official', 'master-key');

    // Read order (backward compatible):
    // 1) Explicit env file, 2) var/ file, 3) legacy repo file (read-only fallback)
    const readCandidates = [
        process.env.WHATSAPP_MASTER_KEY_FILE,
        defaultVarFile,
        defaultRepoFile
    ].filter(Boolean);

    // Write target (never write into tracked repo file)
    const WRITE_MASTER_KEY_FILE = process.env.WHATSAPP_MASTER_KEY_FILE || defaultVarFile;
    ...
    // Tentar arquivos de chave persistente (read)
    for (const candidate of readCandidates) {
        ...
        if (fs.existsSync(candidate)) {
            try {
                ...
                console.warn(`⚠️ Failed to read master key file (${candidate}), trying next`);
            }
        }
    }
    ...
    try {
        try { fs.mkdirSync(path.dirname(WRITE_MASTER_KEY_FILE), { recursive: true }); } catch { /* ignore */ }
        fs.writeFileSync(WRITE_MASTER_KEY_FILE, newKey, { mode: 0o600 }); // Permissões restritivas
