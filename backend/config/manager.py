...
class ConfigManager:
    ...
    CONFIG_FILE = os.environ.get('SKINCOS_CONFIG') or (
        _DEFAULT_CONFIG_FILE if os.path.exists(_DEFAULT_CONFIG_FILE) else _VAR_CONFIG_FILE
    )
    ...
    def _validate_config(self):
        ...
        for field, rules in self._schema.items():
            if rules.get('required', False) and field not in ConfigManager._config:
                ...
                if expected_type and not isinstance(ConfigManager._config[field], expected_type):
    ...
    def _validate_whatsapp_config(self, validation_errors: list):
        ...
        for field in required_fields:
            if field not in whatsapp:
                validation_errors.append(f"Campo obrigatório ausente em whatsapp_config: {field}")
