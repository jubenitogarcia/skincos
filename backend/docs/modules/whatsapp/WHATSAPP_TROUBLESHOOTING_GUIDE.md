...
tail -f frontend/logs/crm_api.out

# Check WhatsApp process logs for specific port
tail -f backend/var/logs/whatsapp/official/instance-1/local_official_1.out

# Search for errors across all logs
grep -r "ERROR\\|FAILED\\|TIMEOUT" frontend/logs/ backend/var/logs/

# Monitor QR generation (based on current system behavior)
grep "QR RECEIVED\\|QR Code generated" frontend/logs/crm_api.out
```
...
# Check backend logs
tail -n 50 frontend/logs/crm_api.out
...
pkill -f "backend/apps/crm-api/server.js"
...
# Check frontend logs
tail -f frontend/logs/crm_web.out
```
...
rm -f backend/var/core/whatsapp_instances.json
rm -rf backend/apps/whatsapp/official-module/sessions/*
...
cp backend/var/core/conversations_store.json conversations_backup.json
cp backend/var/core/messages_store.json messages_backup.json
...
rm backend/var/core/conversations_store.json
rm backend/var/core/messages_store.json
...
2. **Error Logs:**
   ```bash
   tail -n 100 frontend/logs/crm_api.out
   ```
