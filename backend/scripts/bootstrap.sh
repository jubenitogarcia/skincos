...
Flags:
  --core          Instala dependências do core (a0, messaging-whatsapp, actual-server)
...
Módulos suportados:
  a0 | messaging-whatsapp | actual-server | instagram-module
...
Exemplos:
  ...
  ./scripts/bootstrap.sh --module messaging-whatsapp
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --core)
      MODULES+=("a0" "messaging-whatsapp" "actual-server" "instagram-module")
      shift
      ;;
    --all)
      MODULES+=("a0" "messaging-whatsapp" "actual-server" "instagram-module")
