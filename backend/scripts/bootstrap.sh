...
Notas:
  - `whatsapp-official` também instala `modules/whatsapp/whatsapp/official` (cópia local do whatsapp-web.js) usada pelo `modules/whatsapp/whatsapp/official-module`.
  - `crm` também instala dependências da API isoladas em `modules/crm/api` (install menor e mais rápido).
...
pnpm_install() {
  ...
  for m in "${unique_modules[@]}"; do
    case "$m" in
      crm)
        npm_install "$ROOT_DIR/modules/crm/web"
        pnpm_install "$ROOT_DIR/modules/crm/api"
        ;;
      a0) npm_install "$ROOT_DIR/backend/apps/agent-zero" ;;
      whatsapp-official)
        pnpm_install "$ROOT_DIR/modules/whatsapp/whatsapp/official-module"
        # `whatsapp/official` depende de puppeteer (pesado). Por padrão, pulamos o download do Chromium.
        if [[ -d "$ROOT_DIR/modules/whatsapp/whatsapp/official" && -f "$ROOT_DIR/modules/whatsapp/whatsapp/official/package.json" ]]; then
          if [[ $FORCE_INSTALL -eq 0 && -d "$ROOT_DIR/modules/whatsapp/whatsapp/official/node_modules" ]]; then
            echo "[bootstrap] Skip (node_modules already present): modules/whatsapp/whatsapp/official"
          else
            echo "[bootstrap] Node deps: modules/whatsapp/whatsapp/official (pnpm, PUPPETEER_SKIP_DOWNLOAD=1)"
            (
              cd "$ROOT_DIR/modules/whatsapp/whatsapp/official"
              ...
              echo "[bootstrap] ERROR: pnpm is required for modules/whatsapp/whatsapp/official. Install pnpm or enable corepack." >&2
              exit 2
            )
          fi
        else
          echo "[bootstrap] Skip (missing modules/whatsapp/whatsapp/official)"
      fi
      ;;
    whatsapp-gateway) pnpm_install "$ROOT_DIR/modules/whatsapp/whatsapp/gateway" ;;
    whatsapp-stub) npm_install "$ROOT_DIR/modules/whatsapp/whatsapp/stub" ;;
    actual-server) npm_install "$ROOT_DIR/backend/apps/actual-server" ;;
    chat-module)
      npm_install "$ROOT_DIR/modules/whatsapp/whatsapp/chat-module/whatsapp-core"
      npm_install "$ROOT_DIR/modules/whatsapp/whatsapp/chat-module/whatsapp-api"
      npm_install "$ROOT_DIR/modules/whatsapp/whatsapp/chat-module/whatsapp-ui"
      ;;
