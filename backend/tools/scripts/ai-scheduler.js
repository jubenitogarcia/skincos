...
function loadLocalJson(filePath, fallback = null) {
    try {
...
async function main() {
    ...
    const cfg = loadLocalJson(path.join(REPO_ROOT, '.github', 'ai-runner-config.json'), {}) || {};
    const knowledge = loadLocalJson(path.join(REPO_ROOT, 'backend', 'docs', 'ai-knowledge', 'db.json'), []) || [];
