...
const DEFAULT_REPORT_DIR = path.join(BACKEND_DIR, 'var', 'whatsapp', 'testing', 'reports');
const REPORT_DIR = process.env.WHATSAPP_TEST_REPORT_DIR
    ? path.resolve(process.cwd(), process.env.WHATSAPP_TEST_REPORT_DIR)
    : DEFAULT_REPORT_DIR;
...
async function generateTestReport() {
    ...
    const reportPath = path.join(REPORT_DIR, `whatsapp_channels_test_report_${Date.now()}.json`);
...
`;
    ...
    const summaryPath = path.join(REPORT_DIR, `whatsapp_channels_test_summary_${Date.now()}.md`);
