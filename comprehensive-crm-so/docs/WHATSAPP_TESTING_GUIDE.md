...
- **Screenshots**: `./test_evidence/screenshots/`
- **Logs**: `./test_evidence/*.json`
...
node ../whatsapp_channels_enhanced_comprehensive_test.js
```
...
**Output**: Complete test evidence in `./test_evidence/`
...
grep -r "EADDRINUSE" test_evidence/
grep -r "port.*already.*use" test_evidence/
...
jq '.timing.qr' test_evidence/channel_*_results.json

# Look for timeout patterns
grep -r "timeout" test_evidence/ | grep -i qr
```
...
jq '.responseTime' test_evidence/detailed_logs.json | sort -n

# Look for server errors
grep -r "50[0-9]" test_evidence/
```
...
jq '.summary' test_evidence/enhanced_test_results.json

# Channel-by-channel breakdown
jq '.channels' test_evidence/enhanced_test_results.json

# Performance overview
jq '.summary | {averageTestTime, averageResponseTime}' test_evidence/enhanced_test_results.json
```

**2. Detailed Logs (`detailed_logs.json`)**
```bash
# Filter errors only
jq '.[] | select(.level == "ERROR")' test_evidence/detailed_logs.json

# Show pairing simulation steps
jq '.[] | select(.test | contains("pairing"))' test_evidence/detailed_logs.json

# Response time analysis
jq '.[] | select(.data.responseTime) | {timestamp, responseTime: .data.responseTime}' test_evidence/detailed_logs.json
```
...
jq '.' test_evidence/screenshots/channel_1_started.json

# Check for process conflicts
jq '.systemInfo.processes[]' test_evidence/screenshots/channel_*_started.json
```
...
node --max-old-space-size=512 whatsapp_channels_enhanced_comprehensive_test.js
```
...
jq -r '.evidence.detailedLogs[] | select(.data.responseTime) | "\(.timestamp) \(.data.responseTime)"' \
   test_evidence/enhanced_test_results.json > response_times.tsv
...
jq -r '.evidence.stateCaptures[] | "\(.timestamp) \(.systemInfo.memoryUsage.heapUsed)"' \
   test_evidence/enhanced_test_results.json
...
ls -la whatsapp_channels_enhanced_comprehensive_test.js
...
sed -i 's/CHANNELS: Array.from.*/CHANNELS: [1],/' whatsapp_channels_enhanced_comprehensive_test.js
node whatsapp_channels_enhanced_comprehensive_test.js

# Compare multiple runs
for i in {1..3}; do
    echo "Run $i:"
    node whatsapp_channels_enhanced_comprehensive_test.js 2>&1 | grep "SUCCESS\|ERROR" | wc -l
done
```
...
- Permission issues: Check write access to current directory
...
- name: WhatsApp Channel Tests
  run: |
    # Start services
    NODE_ENV=test node src/api/server.js &
    sleep 15
    ...
    # Archive evidence
    tar -czf test-evidence-${{ github.sha }}.tar.gz test_evidence/
...
# Run baseline test
node whatsapp_channels_enhanced_comprehensive_test.js
BASELINE_TIME=$(jq '.summary.averageTestTime' test_evidence/enhanced_test_results.json)
BASELINE_RESPONSE=$(jq '.summary.averageResponseTime' test_evidence/enhanced_test_results.json)

# Move baseline results
mv test_evidence baseline_evidence

# Run comparison test  
node whatsapp_channels_enhanced_comprehensive_test.js
CURRENT_TIME=$(jq '.summary.averageTestTime' test_evidence/enhanced_test_results.json)
CURRENT_RESPONSE=$(jq '.summary.averageResponseTime' test_evidence/enhanced_test_results.json)
...
TEST_FILE="test_evidence/enhanced_test_results.json"
...
echo "- Test Results: \`test_evidence/enhanced_test_results.json\`"
echo "- Detailed Logs: \`test_evidence/detailed_logs.json\`"  
echo "- State Captures: \`test_evidence/screenshots/\`"
```
