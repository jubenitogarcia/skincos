...
# restart_crm.sh (local ao módulo comprehensive-crm-so)
...
#   --crm-api-port PORT    Porta da API Express (default 3100 ou $CRM_API_PORT)
...
# Diretórios
ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"        # comprehensive-crm-so/
CRM_DIR="$ROOT_DIR"                                     # raiz do módulo CRM
...
kill_procs() {
  ...
  # Fallback por nome do arquivo (limitado ao projeto)
  pgrep -fl "src/api/server.js" | grep -q "${CRM_DIR}" && pkill -f "src/api/server.js" 2>/dev/null || true
}
