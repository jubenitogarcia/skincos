...
import { registerAuthRoutes } from '../../server/routes.js'
...
const CONVERSATIONS_FILE = process.env.CRM_CONVERSATIONS_FILE || path.join(process.cwd(), 'conversations_store.json')
let conversations = [
    {
        conversationId: '1',
        status: 'active',
...
// Replit Auth Integration - Setup authentication routes
try {
...
const MESSAGES_FILE = process.env.CRM_MESSAGES_FILE || path.join(process.cwd(), 'messages_store.json')
...
const SUPPRESSION_FILE = process.env.CRM_SUPPRESSION_FILE || path.join(process.cwd(), 'ai_suppression.json')
let aiSuppression = {}
...
const WA_INSTANCES_META_FILE = process.env.CRM_WA_INSTANCES_META || path.join(process.cwd(), 'wa_instances_meta.json')
