...
- `HARMONIA_AUTO_EXECUTE=1` (opcional, cria tasks a partir das `actions` apenas quando o modo `assisted` está explicitamente habilitado)
- `CRM_CONTINUOUS_WORKERS_MODE` (`disabled`, `observe` ou `assisted`; default da API: `disabled`)
- `HARMONIA_WORKER_MODE` (alias compatível; `HARMONIA_WORKER=1` legado mapeia somente para `observe`)
- `CRM_CONTINUOUS_WORKERS_ENABLED=1` (gate do processo dedicado; o launcher falha fechado sem ele)
- `CRM_CONTINUOUS_WORKERS_ASSISTED_CONFIRMED=1` (segunda trava obrigatória para `assisted`)
