...
async function persist(client, { plan }) {
    ...
    await insertJsonChunks(client, guardedAppLinks.map((item) => ({ profile_id: item.profileId, registration_id: item.registrationId, method: item.method, confidence: item.confidence, status: item.status, evidence: item.evidence, run_id: runId })),
    ...
    await insertJsonChunks(client, guardedCaixaLinks.map((item) => ({ profile_id: item.profileId, caixa_customer_id: item.caixaCustomerId, method: item.method, confidence: item.confidence, status: item.status, evidence: item.evidence, run_id: runId })),
