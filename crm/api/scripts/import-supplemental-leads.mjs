...
async function loadInputs(pool) {
    const [apps, customers, sales, canonicalClients, appCaixa, appAttendance, attendanceCaixa] = await Promise.all([
        ...
        pool.query(`select id::text, merged_into_id::text as "mergedIntoId", canonical_name as name from crm_atendimento.canonical_clients`),
        ...
        pool.query(`select l.app_registration_id as "registrationId",l.client_id::text as "attendanceClientId",l.status
            from crm_atendimento.app_registration_attendance_links l
            join crm_atendimento.canonical_clients c on c.id=l.client_id`),
        pool.query(`select l.client_id::text as "attendanceClientId", l.caixa_customer_id::text as "caixaCustomerId", l.status from crm_atendimento.client_caixa_links l join crm_atendimento.canonical_clients c on c.id=l.client_id`),
    ])
...
async function persist(client, { plan }) {
    ...
    const guardedAppLinks = guardAutoConfirmedIdentityLinkProposals({
        proposals: plan.appLinks,
        persistedLinks: persistedLeadAppBefore.rows,
    ...
    const guardedCaixaLinks = guardAutoConfirmedIdentityLinkProposals({
        proposals: plan.caixaLinks,
        persistedLinks: persistedLeadCaixaBefore.rows,
        getSourceId: (link) => link.profileId,
        getTargetId: (link) => link.caixaCustomerId,
    })
    const automaticLinkGuards = {
        leadAppDemoted: guardedLinkCount(guardedAppLinks),
        leadCaixaDemoted: guardedLinkCount(guardedCaixaLinks),
    }
    const run = await client.query(`insert into crm_atendimento.supplemental_lead_import_runs(source_sheet_id,summary) values($1,$2::jsonb) returning id`, [spreadsheetId, JSON.stringify({
        ...plan.summary,
        automaticLinkGuards,
    })])
    ...
    const effectiveAppLinks = await upsertJsonChunksReturning(client, guardedAppLinks.map((item) => ({ profile_id: item.profileId, registration_id: item.registrationId, method: item.method, confidence: item.confidence, status: item.status, evidence: item.evidence, run_id: runId })),
        `insert into crm_atendimento.supplemental_lead_profile_app_links(source_profile_id,app_registration_id,method,confidence,status,evidence,run_id)
         ...
         returning source_profile_id as "profileId",app_registration_id as "registrationId",status`)
    const effectiveCaixaLinks = await upsertJsonChunksReturning(client, guardedCaixaLinks.map((item) => ({ profile_id: item.profileId, caixa_customer_id: item.caixaCustomerId, method: item.method, confidence: item.confidence, status: item.status, evidence: item.evidence, run_id: runId })),
        `insert into crm_atendimento.supplemental_lead_profile_caixa_links(source_profile_id,caixa_customer_id,method,confidence,status,evidence,run_id)
         ...
         returning source_profile_id as "profileId",caixa_customer_id::text as "caixaCustomerId",status`)
    const sourceLinkTransitions = [
        ...collectAutomaticIdentityLinkTransitions({
            effectiveLinks: effectiveAppLinks,
            persistedLinks: persistedLeadAppBefore.rows,
            linkType: 'lead_app',
    ...
    const [currentProfiles, currentRegistrations, currentCanonical, currentCustomers,
        ...
        client.query(`select id::text,merged_into_id::text as "mergedIntoId",canonical_name as name from crm_atendimento.canonical_clients`),
    ...
    const identityProjectionLedger = await recordIdentityProjectionMaterialization(client, {
        origin: 'supplemental_lead_import',
        components,
