...
test('creates the queue and immutable event ledger only after local prerequisite checks', async () => {
    ...
    assert.equal(report.runtimeGrants.length, 5)
    ...
    assert.ok(fixture.calls.some(({ sql }) => /grant select on table crm_atendimento\.schema_migrations to skincos/i.test(sql)))
