...
test('records append-only projection lineage and audit evidence when the reviewed workflow is active', async () => {
    ...
    assert.deepEqual(result, {
        ...
        membersCreated: 1,
        membersMoved: 1,
        sourceLinksActivated: 0,
        sourceLinksDeactivated: 0,
        lineage: 2,
    })
...
test('records only automatic source-link topology transitions', () => {
    ...
    assert.deepEqual(collectAutomaticIdentityLinkTransitions({
        ...configuration,
        persistedLinks: [],
        effectiveLinks: [{ registrationId: 'app-1', caixaCustomerId: 'cash-1', status: 'auto_confirmed' }],
    }), [{
        linkType: 'app_caixa', sourceType: 'app_registration', sourceId: 'app-1',
        targetType: 'caixa_customer', targetId: 'cash-1', transition: 'automatic_activated', resultingStatus: 'auto_confirmed',
    }])
    assert.deepEqual(collectAutomaticIdentityLinkTransitions({
        ...configuration,
        persistedLinks: [{ registrationId: 'app-1', caixaCustomerId: 'cash-1', status: 'auto_confirmed' }],
        effectiveLinks: [{ registrationId: 'app-1', caixaCustomerId: 'cash-1', status: 'suggested' }],
    }), [{
        linkType: 'app_caixa', sourceType: 'app_registration', sourceId: 'app-1',
        targetType: 'caixa_customer', targetId: 'cash-1', transition: 'automatic_deactivated', resultingStatus: 'suggested',
    }])
    assert.deepEqual(collectAutomaticIdentityLinkTransitions({
        ...configuration,
        persistedLinks: [{ registrationId: 'app-1', caixaCustomerId: 'cash-1', status: 'auto_confirmed' }],
        effectiveLinks: [{ registrationId: 'app-1', caixaCustomerId: 'cash-1', status: 'auto_confirmed_spelling' }],
    }), [])
})

test('records a source-link activation even when no physical member moves', async () => {
    const queries = []
    const client = {
        async query(sql, params = []) {
            const normalized = String(sql).replace(/\s+/g, ' ').trim()
    ...
    assert.equal(result.membersMoved, 0)
    assert.equal(result.sourceLinksActivated, 1)
    assert.equal(queries.some(({ sql }) => sql.startsWith('insert into crm_atendimento.identity_member_history')), false)
    assert.equal(queries.some(({ sql }) => sql.startsWith('insert into crm_atendimento.identity_source_link_history')), true)
})
