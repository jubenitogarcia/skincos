...
function reasonReference(secret, reason) {
    return `reason:${assistedHmac(secret, 'commercial-assisted-reason-v1', { reason })}`
}

function phoneHash(secret, phone) {
    return assistedHmac(secret, 'commercial-assisted-phone-v1', { phone: String(phone).replace(/\D/g, '') })
}

function requestHash(secret, operation, actorReferenceValue, payload) {
    return assistedHmac(secret, 'commercial-assisted-request-v1', { operation, actorReference: actorReferenceValue, payload })
}

function correlationHash(secret, operation, payload) {
    return assistedHmac(secret, 'commercial-assisted-correlation-v1', { operation, payload })
}
...
export async function commercialAssistedReadiness(db) {
    ...
    const appendOnlyReady = ['snapshots_immutable', 'templates_immutable', 'attempts_immutable', 'events_immutable', 'receipts_immutable', 'controls_immutable'].every((key) => bool(row[key]))
    ...
    if (relationsReady) {
        const migration = await db.query(`select id from crm_atendimento.schema_migrations
            where id=any($1::text[]) and rolled_back_at is null`, [[COMMERCIAL_ASSISTED_MIGRATION_ID, COMMERCIAL_CANARY_MIGRATION_ID]])
    ...
    return {
        ready: relationsReady && appendOnlyReady && dependenciesReady && migrationReady && canaryReady,
...
async function readSourceHealth(client) {
    const result = await client.query(`select count(distinct source_id)::int as source_count,
        bool_and(last_status='complete' and validated_snapshot_complete is true and reconciliation_required is false and
            validated_at is not null and validated_at >= now()-interval '24 hours') as healthy,
        bool_and(validated_snapshot_complete is true) as snapshots_complete
        from crm_atendimento.clientes_source_operation_checkpoints
        where source_id=any($1::text[])`, [REQUIRED_SOURCE_IDS])
    const row = result.rows[0] || {}
    const sourceCount = Number(row.source_count || 0)
    return {
        status: sourceCount === REQUIRED_SOURCE_IDS.length && bool(row.healthy) ? 'healthy' : 'stale',
        snapshotComplete: sourceCount === REQUIRED_SOURCE_IDS.length && bool(row.snapshots_complete),
        observedSources: sourceCount,
    }
}
...
async function readOffer(client, offerId, unit, identityId) {
    ...
    const procedures = await client.query(`select procedure.id::text as id, procedure.name, offer_procedure.quantity, offer_procedure.quantity_unit
        ...
        where offer_procedure.offer_id=$1::uuid order by offer_procedure.display_order,procedure.name`, [offerId])
...
async function buildContext(client, { actionId, offerId, templateId, actor, secret }) {
    ...
    await client.query(`select pg_advisory_xact_lock(hashtext($1))`, [`commercial-assisted-phone:${recipientPhoneHash}`])
...
async function currentState(client, attemptId) {
    const result = await client.query(`select event_type from crm_atendimento.commercial_assisted_events
        ...
        order by event_order desc limit 1`, [attemptId])
...
export function createCommercialAssistedCommunicationStore({ pool, databaseUrl, auditHmacKey } = {}) {
    ...
    return {
        ...
        async issueHandoff(payload = {}, actor) {
            ...
            return withPgTransaction(pgPool, async (client) => {
                ...
                const attemptResult = await client.query(`select attempt.id::text as id,attempt.action_id::text as action_id,
                    ...
                    where attempt.id=$1::uuid and attempt.actor_reference=$2 for update of attempt`, [attemptId, value])
                ...
                const tokenHash = assistedHmac(keySecret, 'commercial-assisted-handoff-token-v1', { token })
        ...
        async revealHandoff(token, payload = {}, actor) {
            ...
            const keySecret = secret()
            const value = actorRef(keySecret, actor)
            const tokenHash = assistedHmac(keySecret, 'commercial-assisted-handoff-token-v1', { token: suppliedToken })
            return withPgTransaction(pgPool, async (client) => {
                await client.query(`select pg_advisory_xact_lock(hashtext($1))`, [IDENTITY_GRAPH_LOCK_KEY])
                const handoffResult = await client.query(`select handoff.id::text as handoff_id,handoff.attempt_id::text as attempt_id,handoff.state,handoff.expires_at,
                    ...
                    join crm_atendimento.commercial_assisted_offer_snapshots offer_snapshot on offer_snapshot.id=attempt.offer_snapshot_id
                    where handoff.token_hash=$1 and handoff.actor_reference=$2 for update of handoff,attempt`, [tokenHash, value])
        ...
        async processWebhook(payload = {}, actor = { id: 'assisted-webhook', role: 'SERVICE' }) {
            requirePool(pgPool)
            await assertReady(pgPool)
            const event = normalizeWebhookPayload(payload)
            const keySecret = secret()
            const service = `service:${text(actor?.id || 'assisted-webhook').replace(/[^A-Za-z0-9._:-]/g, '').slice(0, 100) || 'assisted-webhook'}`
            const eventHash = assistedHmac(keySecret, 'commercial-assisted-webhook-event-v1', { eventId: event.eventId })
            return withPgTransaction(pgPool, async (client) => {
                await client.query(`select pg_advisory_xact_lock(hashtext($1))`, [IDENTITY_GRAPH_LOCK_KEY])
                const receipt = await client.query(`insert into crm_atendimento.commercial_assisted_webhook_receipts(event_hash,attempt_id,event_type)
                    values ($1,$2::uuid,$3) on conflict(event_hash) do nothing returning event_hash`, [eventHash, event.attemptId, event.eventType])
                if (!receipt.rows[0]?.event_hash) return { accepted: true, deduplicated: true, providerSend: false, externalDispatch: false, safety: publicSafety() }
                const attempt = await client.query(`select attempt.id::text as id,attempt.identity_id::text as identity_id,attempt.action_id::text as action_id,
                    ...
                    from crm_atendimento.commercial_assisted_attempts attempt
                    join crm_atendimento.commercial_assisted_offer_snapshots offer_snapshot on offer_snapshot.id=attempt.offer_snapshot_id
                    where attempt.id=$1::uuid for update of attempt`, [event.attemptId])
                ...
                if (event.eventType !== 'stop' && current === 'confirmed') {
                    throw assistedError('COMMERCIAL_ASSISTED_WEBHOOK_HANDOFF_REQUIRED')
                }
                ...
                await appendEvent(client, {
                    ...
                    actorReference: service,
                    correlation: eventHash,
                    occurredAt: event.occurredAt,
                    payload: { source: 'signed_webhook', eventHash, stopApplied: event.eventType === 'stop', permissionRevision, providerSend: false, externalDispatch: false },
                })
