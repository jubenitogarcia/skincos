...
export const CRM_CORE_PROJECTION_DELTA_BASELINE_CONTRACT = 'atendimento/crm-core/projection-delta-baseline/v1'
export const CRM_CORE_PROJECTION_DELTA_BASELINE_READBACK_CONTRACT = 'atendimento/crm-core/projection-delta-baseline-readback/v1'
...
const MAX_BASELINE_ROWS = 10_000
...
function positiveInteger(value, code) {
    ...
    if (!Number.isSafeInteger(normalized) || normalized < 1 || normalized > MAX_BASELINE_ROWS) fail(code)
    return normalized
}
...
function unitSlugs(value, code) {
    if (!Array.isArray(value) || value.length < 1 || value.length > MAX_BASELINE_ROWS) fail(code)
...
function backfill(value, code) {
    const descriptor = object(value, code)
    exactKeys(descriptor, ['batchId', 'batchDigest', 'capturedAt', 'cursorDigest', 'rowCount', 'unitSlugs', 'eventCount'], code)
...
function receipt(value, code) {
    ...
    const descriptor = object(value, code)
    exactKeys(descriptor, ['contractVersion', 'status', 'batchId', 'batchDigest', 'eventCount', 'target'], code)
    ...
    return Object.freeze({
        ...
        batchDigest: sha256(descriptor.batchDigest, code),
        eventCount: positiveInteger(descriptor.eventCount, code),
...
function readback(value, code) {
    if (value === null) return null
    const descriptor = object(value, code)
    exactKeys(descriptor, ['contract', 'status', 'batchId', 'batchDigest', 'membershipDigest', 'watermark', 'target'], code)
...
function assertRelationships({ targetValue, sourceValue, snapshotValue, backfillValue, seedValue, receiptValue, readbackValue }, code) {
    if (backfillValue.capturedAt !== snapshotValue.capturedAt
        || backfillValue.cursorDigest !== snapshotValue.cursorDigest
        || backfillValue.rowCount !== snapshotValue.rowCount
        ...
        || !same(seedValue.unitSlugs, snapshotValue.unitSlugs)) fail(code)
    if (receiptValue && (receiptValue.batchId !== backfillValue.batchId
        || receiptValue.batchDigest !== backfillValue.batchDigest
        || receiptValue.eventCount !== backfillValue.eventCount
        || !same(receiptValue.target, targetValue))) fail(code)
    if (readbackValue && (readbackValue.batchId !== backfillValue.batchId
        || readbackValue.batchDigest !== backfillValue.batchDigest
        || readbackValue.membershipDigest !== seedValue.membershipDigest
        || readbackValue.watermark !== snapshotValue.watermark
        || !same(readbackValue.target, targetValue))) fail(code)
    // The source and target key ids are intentionally distinct fields. A
    // rotation can change either key without changing the handoff snapshot.
...
export function assertAtendimentoProjectionDeltaBaseline(value) {
    ...
    exactKeys(baseline, ['contract', 'state', 'target', 'source', 'snapshot', 'backfill', 'seed', 'receipt', 'readback'], 'ATENDIMENTO_CRM_PROJECTION_DELTA_BASELINE_INVALID')
    ...
    const receiptValue = receipt(baseline.receipt, 'ATENDIMENTO_CRM_PROJECTION_DELTA_BASELINE_INVALID')
...
export function createAtendimentoProjectionDeltaBaselineSeed({ rows, capturedAt } = {}) {
    if (!Array.isArray(rows) || rows.length < 1 || rows.length > MAX_BASELINE_ROWS) fail('ATENDIMENTO_CRM_PROJECTION_DELTA_BASELINE_SEED_INVALID')
    const normalizedRows = rows.map(assertAtendimentoProjectionMembershipRow).sort((left, right) => (
...
export function acceptAtendimentoProjectionDeltaBaseline(baseline, suppliedReceipt) {
    ...
    const acceptedReceipt = receipt(suppliedReceipt, 'ATENDIMENTO_CRM_PROJECTION_DELTA_BASELINE_RECEIPT_INVALID')
    if (!acceptedReceipt) fail('ATENDIMENTO_CRM_PROJECTION_DELTA_BASELINE_RECEIPT_INVALID')
    return assertAtendimentoProjectionDeltaBaseline({ ...current, state: CRM_CORE_PROJECTION_DELTA_BASELINE_STATES.ACCEPTED, receipt: acceptedReceipt })
}
