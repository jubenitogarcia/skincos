...
// Source writers use the rows returned by their upsert, rather than their
// proposal, because a protected human terminal state can change the effective
// persisted status. The ledger records automatic topology activation and
// deactivation, not an unchanged refresh or a status-only confidence change.
export function collectAutomaticIdentityLinkTransitions({
    effectiveLinks = [],
    persistedLinks = [],
    linkType,
    sourceType,
    ...
    if (!String(linkType || '').trim() || !String(sourceType || '').trim() || !String(targetType || '').trim()
        || typeof getSourceId !== 'function' || typeof getTargetId !== 'function'
        || typeof normalizeSourceId !== 'function' || typeof normalizeTargetId !== 'function') {
        throw new TypeError('IDENTITY_LINK_TRANSITION_RESOLVERS_REQUIRED')
    }
    const previousStatusByPair = new Map()
    for (const link of persistedLinks) {
        const sourceId = normalizedLinkValue(normalizeSourceId(getSourceId(link)))
        const targetId = normalizedLinkValue(normalizeTargetId(getTargetId(link)))
        if (sourceId && targetId) previousStatusByPair.set(`${sourceId}\u0000${targetId}`, normalizedLinkValue(link?.status))
    }
    const transitions = new Map()
    for (const link of effectiveLinks) {
        const status = normalizedLinkValue(link?.status)
        const sourceId = normalizedLinkValue(normalizeSourceId(getSourceId(link)))
        const targetId = normalizedLinkValue(normalizeTargetId(getTargetId(link)))
        if (!sourceId || !targetId) continue
        const key = `${sourceId}\u0000${targetId}`
        const previousStatus = previousStatusByPair.get(key)
        const wasActive = AUTOMATIC_IDENTITY_LINK_STATUSES.has(previousStatus)
        const isActive = AUTOMATIC_IDENTITY_LINK_STATUSES.has(status)
        ...
        transitions.set(key, {
            ...
            targetId,
            transition: isActive ? 'automatic_activated' : 'automatic_deactivated',
            resultingStatus: status,
        })
    }
    return [...transitions.values()].sort((left, right) => left.linkType.localeCompare(right.linkType)
...
async function identityReviewLedgerStatus(client) {
    ...
    const migration = row.registry
        ...
        : { rows: [] }
    const appliedMigrationIds = new Set(migration.rows.map((entry) => String(entry?.id || '').trim()))
...
export async function recordIdentityProjectionMaterialization(client, {
    ...
    const orderedSourceLinkTransitions = sourceLinkTransitions.map((link) => ({
        linkType: normalizedLinkValue(link?.linkType),
        sourceType: normalizedLinkValue(link?.sourceType),
        sourceId: normalizedLinkValue(link?.sourceId),
        targetType: normalizedLinkValue(link?.targetType),
        targetId: normalizedLinkValue(link?.targetId),
        transition: normalizedLinkValue(link?.transition),
        resultingStatus: normalizedLinkValue(link?.resultingStatus),
    })).filter((link) => link.linkType && link.sourceType && link.sourceId && link.targetType && link.targetId
        && ['automatic_activated', 'automatic_deactivated'].includes(link.transition) && link.resultingStatus)
        .sort((left, right) => left.linkType.localeCompare(right.linkType)
            || left.sourceType.localeCompare(right.sourceType) || left.sourceId.localeCompare(right.sourceId)
            || left.targetType.localeCompare(right.targetType) || left.targetId.localeCompare(right.targetId))
    if (!changes.length && !orderedSourceLinkTransitions.length) return { available: false, recorded: false, membersCreated: 0, membersMoved: 0, sourceLinksActivated: 0, sourceLinksDeactivated: 0 }
    const ledger = await identityReviewLedgerStatus(client)
    if (!ledger.ready) {
        if (orderedSourceLinkTransitions.length && ledger.workflowV1Active) {
            throw identityProjectionError('IDENTITY_PROJECTION_SOURCE_LINK_LEDGER_REQUIRED')
        }
    ...
    const inputFingerprint = createHash('sha256').update(JSON.stringify({
        ...
        sourceLinkTransitions: orderedSourceLinkTransitions,
    })).digest('hex')
    const summary = {
        origin: String(origin || 'persisted_identity_projection'),
        membersCreated,
        membersMoved,
        sourceLinksActivated: orderedSourceLinkTransitions.filter((link) => link.transition === 'automatic_activated').length,
        sourceLinksDeactivated: orderedSourceLinkTransitions.filter((link) => link.transition === 'automatic_deactivated').length,
        sourceLinkTypes: uniqueStrings(orderedSourceLinkTransitions.map((link) => link.linkType)),
    ...
    if (orderedSourceLinkTransitions.length) {
        await client.query(`insert into crm_atendimento.identity_source_link_history(
            ...
            runId,
            JSON.stringify(orderedSourceLinkTransitions.map((link) => ({
                ...
                resulting_status: link.resultingStatus,
            }))),
            summary.origin,
        ])
    }
    ...
    return {
        ...
        membersCreated,
        membersMoved,
        sourceLinksActivated: summary.sourceLinksActivated,
        sourceLinksDeactivated: summary.sourceLinksDeactivated,
