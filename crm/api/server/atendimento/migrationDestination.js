...
export const PRODUCTION_DATABASE_NAME = 'skincos_clientes_production'
export const PRODUCTION_MIGRATOR_USER = 'skincos_clientes_migrator_login'
export const PRODUCTION_OWNER_ROLE = 'skincos_clientes_owner'
...
export function isStrictAtendimentoMigrationDestination(databaseUrl, target = ATENDIMENTO_MIGRATION_TARGETS.LOCAL) {
    ...
    const databaseName = target === ATENDIMENTO_MIGRATION_TARGETS.STAGING
        ? STAGING_DATABASE_NAME
        : PRODUCTION_DATABASE_NAME
    const migratorUser = target === ATENDIMENTO_MIGRATION_TARGETS.STAGING
        ? STAGING_MIGRATOR_USER
        : PRODUCTION_MIGRATOR_USER
    ...
    try {
        ...
        return url.protocol === 'postgresql:' &&
            ...
            (url.port || '5432') === '5432' &&
            decodeURIComponent(url.username || '') === migratorUser &&
...
export async function assertAtendimentoMigrationDestination(client, databaseUrl, target = ATENDIMENTO_MIGRATION_TARGETS.LOCAL) {
    ...
    const managedTarget = [ATENDIMENTO_MIGRATION_TARGETS.STAGING, ATENDIMENTO_MIGRATION_TARGETS.PRODUCTION].includes(target)
    const migratorUser = target === ATENDIMENTO_MIGRATION_TARGETS.STAGING
        ? STAGING_MIGRATOR_USER
        : PRODUCTION_MIGRATOR_USER
    const ownerRole = target === ATENDIMENTO_MIGRATION_TARGETS.STAGING
        ? STAGING_OWNER_ROLE
        : PRODUCTION_OWNER_ROLE
    const expected = target === ATENDIMENTO_MIGRATION_TARGETS.STAGING
        ...
        : target === ATENDIMENTO_MIGRATION_TARGETS.PRODUCTION
            ? { database: PRODUCTION_DATABASE_NAME, user: PRODUCTION_OWNER_ROLE }
    ...
    if (managedTarget && row.session_user === migratorUser && row.database_user === migratorUser) {
...
export function migrationDestinationLabel(target = ATENDIMENTO_MIGRATION_TARGETS.LOCAL) {
    if (target === ATENDIMENTO_MIGRATION_TARGETS.STAGING) return STAGING_DATABASE_NAME
    if (target === ATENDIMENTO_MIGRATION_TARGETS.PRODUCTION) return PRODUCTION_DATABASE_NAME
    return 'skincos_crm_local'
}
