...
async function withCommercialContactTransaction(pgPool, operation) {
    // Serializable isolation adds predicate protection for the Harmonia phone
    // lookup, so a newly inserted opt-out cannot be silently missed between
    // the eligibility read and the action update. PostgreSQL can signal a
    // retryable serialization conflict under contention; retry once before
