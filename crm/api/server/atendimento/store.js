...
async function identityReviewWorkflowStatus(pgPool) {
    ...
    if (!row.registry || !row.decisions || !row.runs || !row.member_history || !row.lineage
        || !row.member_history_event_order || !row.decision_resulting_status || !row.decision_event_order) return { ready: false }
    ...
    return { ready: !!migration.rows[0]?.id }
}
...
async function assertNoDependentIdentityReviewDecision(client, decision) {
    if (!decision?.materialization_run_id || decision.decision !== 'confirmed') return
    const originalRun = await client.query(`select summary from crm_atendimento.identity_materialization_runs
        where id=$1::uuid for share`, [decision.materialization_run_id])
    const summary = originalRun.rows[0]?.summary || {}
    ...
    // The graph is shared across all source types.  A later still-active
    // confirmation involving this run's identities makes an old reversal
    // non-local; reverse that dependent decision first instead of splitting a
    // transitive component behind its recorded source state.
    const dependent = await client.query(`with latest_decisions as (
            select distinct on (review_type,source_id,target_id)
    ...
    if (dependent.rows[0]) throw identityReviewError('IDENTITY_REVIEW_UNDO_DEPENDENT_DECISION', 409)
}
    ...
    return {
        ...
        async undoIdentityReviewDecision(payload, actor) {
            ...
            return withPgTransaction(pgPool, async (client) => {
                ...
                await assertNoDependentIdentityReviewDecision(client, latest)
