...
import { hasModulePermission, moduleRegistry as modules, ModuleSlot, modulesByKey } from '@/modules/registry'
...
export default function AppFunctionalNeatlab() {
    ...
    const hasModuleAccess = React.useCallback(
        (moduleKey: string) => {
            ...
            return hasModulePermission(module, { role: roleKey, allowedModules: user?.allowedModules, financeEnabled })
        },
	                        ...
	                        <div className={`flex-1 overflow-y-auto ${sidebarExpanded ? 'p-4' : 'p-2'} space-y-2`}>
	                            ...
	                            {filteredModules.map((m, index) => (
	                                (() => {
	                                    ...
	                                    return (
                                        <TooltipLabel key={m.key} label={m.label} description={isLocked ? m.unavailable.description : undefined}>
                                            <button
                                                onPointerDown={() => {
                                                    if (isLocked) return
                                                    selectModule(m.key)
                                ...
                                <div className={`animate-fade-in ${active === 'conversa' ? 'flex h-full min-h-0 flex-col' : ''}`}>
                                    {mountedModuleKeys
                                        ...
                                        .map((m) => {
                                            ...
                                            return <ModuleSlot key={moduleEntry.key} module={moduleEntry} className={active === 'conversa' ? 'h-full min-h-0' : active === 'site-tracking' ? 'min-w-0 max-w-full overflow-x-hidden' : undefined} />
                                        })}
                                </div>
                                <ContextDebugger />
                            </div>
