...
function commercialOperationToken() {
...
function ActionForm({ detail, units, professionals, onSaved }: { detail: CommercialProfileDetail; units: Array<{ slug: string; name: string }>; professionals: Array<{ name: string }>; onSaved: () => Promise<void> }) {
  ...
  const save = async () => {
    try {
      ...
      const result = await createCommercialAction({ identityId: detail.profile.identityId, segmentKey: firstSegment?.key || 'manual_follow_up', actionType, contactChannel: 'whatsapp', owner, dueDate, notes, unit: unit || (units.length === 1 ? units[0].slug : undefined), idempotencyKey: commercialOperationToken() })
...
function ActionHistory({ actions, contactEligibility, contactRolloutAllowed, onUpdated }: { actions: CommercialAction[]; contactEligibility: CommercialProfile['contactEligibility']; contactRolloutAllowed: boolean; onUpdated: () => Promise<void> }) {
  ...
  const changeStatus = async (action: CommercialAction, status: CommercialAction['status']) => {
    try {
      ...
      const result = await updateCommercialAction(action.id, { status, expectedRevision: action.revision, idempotencyKey: commercialOperationToken(), reason: 'Atualização operacional assistida' })
...
export function ClientCommercialModule() {
  ...
  const showProfileWorkspace = workspaceView === 'overview' || workspaceView === 'wallet'
  ...
  return <section className="space-y-6 text-white">
    ...
    {workspaceView === 'actions' ? <CommercialOperationsPanel units={units} professionals={professionals} onOpenIdentity={(identityId) => { void loadDetail(identityId, overview?.asOf) }} /> : null}
