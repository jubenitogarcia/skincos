...
export function ConversaModule() {
  const { user } = useAuth()
  ...
  const canConversa = hasModuleAccess('conversa')
  ...
  const publishHeaderState = useCallback((state: ConversaHeaderState) => {
    try {
      window.dispatchEvent(new CustomEvent('skincos:conversa:header', { detail: state }))
    } catch {
      /* ignore */
    }
  }, [])
  ...
  useEffect(() => {
    return () => {
      try {
        window.dispatchEvent(new CustomEvent('skincos:conversa:header', { detail: null }))
      } catch {
        /* ignore */
      }
    }
  ...
  if (!canOmnichannel) {
    return (
      <Card className="glass-card">
        <CardHeader>
          <CardTitle className="text-white">Sem acesso ao módulo de Conversa</CardTitle>
        </CardHeader>
  ...
  return (
    <div className="h-full min-h-0 conversa-surface">
      <div className="h-full min-h-0">
        {renderActivePanel()}
      </div>
    </div>
