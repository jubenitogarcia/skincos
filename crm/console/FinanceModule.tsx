...
export function FinanceModule() {
  ...
  const submitMovement = () => runAction('movement', async () => {
    ...
    await financeApi.create('/movements', scopeId, {
    ...
    setMovementOpen(false); setMovementDraft(emptyMovement())
  })
...
function RegistrationCard({ title, description, items, onAdd, canOperate, extra }: { title: string; description: string; items: any[]; onAdd: () => void; canOperate: boolean; extra?: (item: any) => string }) { return <Card className="glass-card"><CardHeader><div className="flex items-start justify-between gap-3"><div><CardTitle>{title}</CardTitle><CardDescription>{description}</CardDescription></div>{canOperate ? <Button size="sm" variant="outline" onClick={onAdd}><Plus size={16} />Adicionar</Button> : null}</div></CardHeader><CardContent>{items.length ? <div className="max-h-52 space-y-2 overflow-y-auto">{items.map((item) => <div key={item.id} className="flex items-center justify-between border-b pb-2 text-sm"><span className="font-medium">{item.name}</span>{extra ? <span className="text-muted-foreground">{extra(item)}</span> : null}</div>)}</div> : <p className="py-4 text-sm text-muted-foreground">Nenhum cadastro neste escopo.</p>}</CardContent></Card> }
