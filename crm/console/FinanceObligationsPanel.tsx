...
function ObligationPlanningSummary({ summary }: { summary: FinanceObligationSummary | null }) {
  ...
  return <Card className="glass-card"><CardHeader><CardTitle>Posição e previsão de 30 dias</CardTitle><CardDescription>Valores em aberto por moeda, calculados no servidor. Previsões não criam lançamentos de caixa.</CardDescription></CardHeader><CardContent className="space-y-5">{totals.length ? <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{totals.map((total) => <div className="rounded-md border p-4" key={`${total.currency}-${total.kind}`}><p className="text-sm text-muted-foreground">{total.kind === 'payable' ? 'A pagar' : 'A receber'} · {total.currency}</p><p className="mt-1 text-xl font-semibold">{money(total.open_minor, total.currency)}</p><p className="mt-2 text-sm text-rose-700">Vencido: {money(total.overdue_minor, total.currency)}</p><p className="text-sm text-muted-foreground">Até {summary.horizonEnd}: {money(total.due_within_horizon_minor, total.currency)}</p></div>)}</div> : <p className="text-sm text-muted-foreground">Não há títulos em aberto para compor a posição e a previsão.</p>}<section><h3 className="font-semibold">Calendário financeiro</h3><p className="mb-3 text-sm text-muted-foreground">Agenda prevista de {summary.asOf} até {summary.horizonEnd}; cada valor continua ligado ao título de origem.</p>{summary.schedule.length ? <Table><TableHeader><TableRow><TableHead>Data prevista</TableHead><TableHead>Fluxo</TableHead><TableHead>Moeda</TableHead><TableHead className="text-right">Valor em aberto</TableHead></TableRow></TableHeader><TableBody>{summary.schedule.map((item) => <TableRow key={`${item.planned_date}-${item.kind}-${item.currency}`}><TableCell>{item.planned_date}</TableCell><TableCell>{item.kind === 'payable' ? 'Saída prevista' : 'Entrada prevista'}</TableCell><TableCell>{item.currency}</TableCell><TableCell className={item.kind === 'payable' ? 'text-right font-semibold text-rose-700' : 'text-right font-semibold text-emerald-700'}>{money(item.amount_minor, item.currency)}</TableCell></TableRow>)}</TableBody></Table> : <p className="rounded-md border border-dashed p-3 text-sm text-muted-foreground">Nenhum título aberto está previsto para a janela selecionada.</p>}</section></CardContent></Card>
}
