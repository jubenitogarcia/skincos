...
export function UsersModule() {
  ...
  return <div className="p-6 space-y-6"><div className="max-w-6xl mx-auto space-y-4">
    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><h1 className="text-lg font-semibold text-white">Usuários</h1><p className="text-sm text-blue-100/70">Cadastro hierárquico com escopos e módulos derivados no servidor.</p></div><div className="flex gap-2"><Button variant="secondary" onClick={() => void load()} disabled={loading}>Recarregar</Button><Button variant="outline" onClick={() => { setForm((current) => ({ ...current, units: selectableUnits.length === 1 ? selectableUnits : [] })); setOpen(true) }} disabled={!canManage}>Cadastrar funcionário</Button></div></div>
