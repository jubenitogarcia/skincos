...
test.describe('atendimento', () => {
  ...
  test('renders dashboard and supports create and inline edit flows', async ({ page }) => {
    ...
    await page.getByTestId('atendimento-multiplier-distribution-trigger').hover()
    const distributionTooltip = page.getByRole('tooltip').filter({ hasText: 'proporção da faixa' })
