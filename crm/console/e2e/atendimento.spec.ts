...
test.describe('atendimento', () => {
  ...
  test('renders dashboard and supports create and inline edit flows', async ({ page }) => {
    ...
    const recordsToggle = page.getByTestId('atendimento-records-toggle')
    const chartsToggle = page.getByTestId('atendimento-charts-toggle')
    const layoutToggle = page.getByTestId('atendimento-header-layout-toggle')
    await expect(analysisToggle).toHaveAttribute('aria-expanded', 'false')
    await expect(recordsToggle).toHaveAttribute('aria-expanded', 'true')
    await expect(page.getByTestId('atendimento-analysis-collapsed')).toBeVisible()
    await expect(layoutToggle).toHaveAttribute('aria-label', 'Expandir tudo')
    await expect(page.getByTestId('atendimento-header-expand-all')).toHaveCount(0)
    await expect(page.getByTestId('atendimento-header-collapse-all')).toHaveCount(0)
    await layoutToggle.click()
    await expect(layoutToggle).toHaveAttribute('aria-label', 'Contrair tudo')
    ...
    await expect(layoutToggle).toHaveAttribute('aria-label', 'Expandir tudo')
    await expect(analysisToggle).toHaveAttribute('aria-expanded', 'false')
    await expect(recordsToggle).toHaveAttribute('aria-expanded', 'false')
    await expect(chartsToggle).toHaveAttribute('aria-expanded', 'false')
    await expect(page.getByTestId('atendimento-records-collapsed')).toBeVisible()
    ...
    await expect(page.getByTestId('atendimento-analysis-header')).toContainText('Indicadores, desempenho por doutor, ranking, metas e faixas são carregados somente quando necessários.')
    await expect(analysisToggle).toHaveAttribute('aria-label', 'Contrair análise')
    ...
    await expect(page.getByTestId('atendimento-conversion-distribution')).toContainText('Meta')
    ...
    const metricGrid = page.getByTestId('atendimento-conversion-metrics-grid')
    const metricGridColumns = await metricGrid.evaluate((element) => getComputedStyle(element).gridTemplateColumns.trim().split(/\s+/).filter(Boolean).length)
    expect(metricGridColumns).toBe(7)
    ...
    await page.getByText('Meta', { exact: true }).hover()
    ...
    await expect(intervalTooltip).toContainText('Intervalo = Desvio Padrão')
    await expect(intervalTooltip).not.toContainText('*')
    ...
    const multiplierCurve = page.getByLabel('Curva do multiplicador pela homogeneidade')
    await expect(multiplierCurve).toHaveCSS('height', '128px')
    const multiplierCurveBox = await multiplierCurve.boundingBox()
    expect(multiplierCurveBox).not.toBeNull()
    if (multiplierCurveBox) expect(multiplierCurveBox.width).toBeLessThanOrEqual(416)
    ...
    const distributionTooltip = page.getByTestId('atendimento-reference-tooltip-lower-side')
    await expect(distributionTooltip).toContainText('Lado inferior')
    await expect(distributionTooltip).toContainText('Fórmula: proporção da faixa')
    await expect(distributionTooltip).toContainText('Nível 0')
    const distributionTooltipBox = await distributionTooltip.evaluate((element) => {
      const bounds = element.getBoundingClientRect()
      return { width: bounds.width, height: bounds.height }
    })
    expect(distributionTooltipBox.width / distributionTooltipBox.height).toBeGreaterThanOrEqual(0.5)
    expect(distributionTooltipBox.width / distributionTooltipBox.height).toBeLessThanOrEqual(2)
    ...
    await expect(importExportTrigger).toHaveAttribute('aria-label', 'Importar / exportar Atendimento')
    await expect(page.getByTestId('atendimento-header-report')).toHaveCount(0)
    await expect(page.getByTestId('atendimento-header-import')).toHaveCount(0)
    ...
    const tableScrollMaxHeight = await page.getByTestId('atendimento-table-scroll').evaluate((element) => Number.parseFloat(getComputedStyle(element).maxHeight))
    expect(tableScrollMaxHeight).toBeGreaterThanOrEqual(600)
    ...
    const importExportMenu = page.getByRole('menu')
    await expect(importExportMenu).toContainText('Importar / exportar')
    await expect(importExportMenu.getByTestId('atendimento-header-report')).toBeVisible()
    await expect(importExportMenu.getByTestId('atendimento-header-import')).toBeVisible()
    await importExportMenu.getByTestId('atendimento-header-import').click()
    ...
    await importExportTrigger.click()
    await page.getByRole('menu').getByTestId('atendimento-header-report').click()
