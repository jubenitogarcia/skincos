...
test.describe('atendimento', () => {
  ...
  test('renders dashboard and supports create and inline edit flows', async ({ page }) => {
    ...
    await page.getByTestId('atendimento-header-collapse-all').click()
    await expect(analysisToggle).toHaveAttribute('aria-expanded', 'false')
    ...
    await page.getByTestId('atendimento-header-expand-all').click()
    ...
    const intervalInfo = page.locator('span', { hasText: 'Intervalo' }).locator('svg.lucide-info').first()
    await expect(intervalInfo).toBeVisible()
    const intervalInfoOverflow = await intervalInfo.evaluate((icon) => getComputedStyle(icon.parentElement as HTMLElement).overflow)
    expect(intervalInfoOverflow).toBe('visible')
    ...
    await expect(page.getByTestId('atendimento-conversion-distribution')).not.toContainText('Resumo')
    ...
    await expect(page.getByRole('button', { name: /Detalhes de (Resumo|Faixas)/ })).toHaveCount(0)
    ...
    await expect(analysisToggle).toContainText(
      'Desempenho por doutor: ranking, totais, metas e faixas do período',
    )
    ...
    const intervalTooltip = page
      .getByRole('tooltip')
      .filter({ hasText: 'Fórmula' })
    await expect(intervalTooltip).toContainText('intervalo = desvio_padrão_amostral')
    ...
    await expect(multiplierDetails).not.toContainText('Lado inferior')
    ...
    await expect(distributionTooltip).toContainText('Lado superior')
    ...
    await expect(dailyGoalTooltip).toContainText('Fórmula')
    await expect(dailyGoalTooltip).toContainText('Aplicação atual: R$ 10,00 ÷ 1 dia = R$ 10,00')
