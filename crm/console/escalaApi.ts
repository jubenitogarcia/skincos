...
export async function fetchAtendimentoEscalaFeed() {
  try {
    const res = await fetch('/api/atendimento/management/feeds/escala', {
      credentials: 'include',
...
export async function importAtendimentoEscala(payload: { feed: any; dryRun?: boolean; commit?: boolean; force?: boolean }) {
  return apiWrite<EscalaAtendimentoImportResult>(`/admin/import/atendimento`, 'POST', payload)
}

export async function syncAtendimentoEscala(options: { commit?: boolean; force?: boolean } = {}) {
  const feed = await fetchAtendimentoEscalaFeed()
  if (!feed.ok) return feed as ApiResponse<EscalaAtendimentoImportResult>
  return importAtendimentoEscala({
    feed: feed.feed,
