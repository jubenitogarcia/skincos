...
export const financeApi = {
  ...
  stageCsv: (scopeId: string, filename: string, csv: string, options: { mapping?: Record<string, string>; encoding?: string; sourceType?: 'generic' | 'moneywiz'; idempotencyKey?: string } = {}) => request<any>(`/imports?${scopedQuery(scopeId)}`, { method: 'POST', headers: idempotencyHeaders(options.idempotencyKey), body: JSON.stringify({ filename, csv, mapping: options.mapping, encoding: options.encoding, sourceType: options.sourceType || 'generic' }) }),
