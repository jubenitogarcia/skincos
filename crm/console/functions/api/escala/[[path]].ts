...
async function handleLocalEscalaRequest(
  ...
  if (rest === '/prefill' && method === 'GET') {
    if (unit && !canUseUnit(actor, unit)) return done(403, { ok: false, error: 'FORBIDDEN_UNIT' })
    return done(200, {
      ok: true,
      suggestions: [],
      windowMonths: month ? [month] : [],
      source: 'local-mock',
    })
  }
...
export async function onRequest(context: any): Promise<Response> {
  ...
  const actorKey = sanitizeEnvSecret(context?.env?.ESCALA_ACTOR_HMAC_KEY)
