...
# Diretórios
CRM_DIR="$(cd "$(dirname "$0")" && pwd)"                # crm/console/
ROOT_DIR="$CRM_DIR"
...
resolve_skincos_root() {
  ...
  for candidate in "$base" "$base/.." "$base/../.." "$base/../../.."; do
    if [[ -d "$candidate/crm/api" && -d "$candidate/crm/console" && -f "$candidate/crm/console/package.json" ]]; then
      (cd "$candidate" && pwd)
      return 0
    fi
  done
...
# API deps are isolated in crm/api for faster installs
if [[ $START_API -eq 1 ]]; then
    if [[ ! -f "$API_DIR/package.json" ]]; then
      ...
      if [[ $api_needs_install -eq 1 ]]; then
        echo "[restart_crm] Instalando dependências da API (isoladas em crm/api)..."
        (
...
start_api() {
  if [[ $START_API -eq 1 ]]; then
    # Local development uses the same native engine as production. Credentials
    # must be supplied by the caller's private environment; never read them
    # from a repository checkout or a retired runtime path.
    export WA_ORCHESTRATOR_PROVIDER="${WA_ORCHESTRATOR_PROVIDER:-evolution}"
