...
if [[ -n "${LOCAL_WA_ORCHESTRATOR_BASIC_AUTH:-}" ]]; then
  PAGES_BINDING_ARGS+=(--binding "WA_ORCHESTRATOR_BASIC_AUTH=${LOCAL_WA_ORCHESTRATOR_BASIC_AUTH}")
fi
if [[ -n "${LOCAL_FINANCE_API_TARGET:-}" ]]; then
  PAGES_BINDING_ARGS+=(--binding "FINANCE_API_TARGET=${LOCAL_FINANCE_API_TARGET}")
fi
for finance_key in LOCAL_FINANCE_ACTOR LOCAL_FINANCE_CSRF_TOKEN; do
  finance_value="${!finance_key:-}"
  if [[ -n "$finance_value" ]]; then
    PAGES_BINDING_ARGS+=(--binding "${finance_key}=${finance_value}")
  fi
done
for runtime_key in \
  LOCAL_RUNTIME_SESSION_ID LOCAL_RUNTIME_WORKTREE LOCAL_RUNTIME_COMMIT LOCAL_RUNTIME_FINGERPRINT \
  LOCAL_RUNTIME_STARTED_AT LOCAL_RUNTIME_MODULE LOCAL_RUNTIME_PAGES_PORT LOCAL_RUNTIME_VITE_PORT \
  LOCAL_RUNTIME_WA_PORT LOCAL_RUNTIME_INSUMOS_PORT LOCAL_RUNTIME_WA_MODE \
  LOCAL_RUNTIME_WA_LOCAL_STUB LOCAL_RUNTIME_WA_TARGET; do
  runtime_value="${!runtime_key:-}"
  if [[ -n "$runtime_value" ]]; then
    PAGES_BINDING_ARGS+=(--binding "${runtime_key}=${runtime_value}")
  fi
done
