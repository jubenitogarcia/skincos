...
describe('CRM local launch catalog', () => {
  it('is the exact local release list and stays in registry parity', () => {
    expect(localLaunchCatalog.modules).toHaveLength(14)
  ...
  it('uses the same role truth for the 14 Gestor and 2 Consultor combinations', () => {
    ...
    expect(gestorModules).toHaveLength(14)
    expect(consultorModules.map((entry) => entry.key)).toEqual(['atendimento', 'ponto'])
  })
