...
import { sanitizeEnvSecret } from "./functions/_lib/envPlaceholders";
...
// Ponto loads these only after the user opens face recognition. The bundle check
// below enforces a separate, stricter budget for the initial CRM shell.
const SPECIALIZED_DEFERRED_CHUNK_LIMIT_KIB = 1400
...
const escalaActorHmacKey = sanitizeEnvSecret(
...
export default defineConfig({
  ...
  build: {
    rollupOptions: {
      output: {
        manualChunks(id) {
          if (id.includes('/node_modules/@tensorflow/')) return 'ponto-tensorflow'
          if (id.includes('/node_modules/@vladmandic/face-api/')) return 'ponto-face-api'
          if (id.includes('/node_modules/@zxing/')) return 'insumos-barcode'
    ...
    chunkSizeWarningLimit: SPECIALIZED_DEFERRED_CHUNK_LIMIT_KIB,
  },
  optimizeDeps: {
    include: [
      'react',
