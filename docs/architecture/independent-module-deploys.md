...
- Runtime CRM: criar um unit de Atendimento independente, com porta/health próprios, `CRM_DOMAIN=atendimento`, `CRM_MODULE_CONTROL_FILE` privado e os identificadores de comando allowlisted `CRM_ATENDIMENTO_DEPLOY_COMMAND`, `CRM_ATENDIMENTO_ROLLBACK_COMMAND`, `CRM_ATENDIMENTO_CONTROL_COMMAND`, além de `CRM_ATENDIMENTO_HEALTH_URL`, configurados no Environment GitHub correspondente. Esses identificadores não são linhas de shell: o workflow de preparação apenas os valida e um executor nativo futuro deve resolvê-los por allowlist fixa, sem `eval` ou fallback para o CRM compartilhado.
