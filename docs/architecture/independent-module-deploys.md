...
5. Configurar o unit/service dedicado do Atendimento, remover seu mount do processo CRM compartilhado e validar health por ambiente. O staging isolado agora é provisionado pelos scripts versionados; só então habilitar `ENABLE_ATENDIMENTO_DEPLOY=true` naquele Environment.
...
- Runtime CRM: o staging usa o unit `crm-atendimento-staging.service`, com porta/health próprios, `CRM_DOMAIN=atendimento`, `CRM_MODULE_CONTROL_FILE` privado e banco/role `skincos_staging`; os identificadores allowlisted `CRM_ATENDIMENTO_DEPLOY_COMMAND`, `CRM_ATENDIMENTO_ROLLBACK_COMMAND`, `CRM_ATENDIMENTO_CONTROL_COMMAND` e `CRM_ATENDIMENTO_HEALTH_URL` são atestados no Environment GitHub. Esses identificadores não são linhas de shell: os scripts nativos versionados os resolvem por allowlist fixa, sem `eval` ou fallback para o CRM compartilhado.
