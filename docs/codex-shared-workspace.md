...
- resolve código comum pela release imutável `/opt/skincos/current/source`; o
  CRM passa a ter contrato próprio em
  `/opt/skincos/current/crm-service`, mas essa troca permanece bloqueada até a
  custódia e o bootstrap dedicados descritos em
  [crm-native-publisher.md](runbooks/crm-native-publisher.md);
