...
Installed from official npm registries after checking official documentation: `@axe-core/playwright`, Testing Library, Storybook React/Vite with a11y/MCP addons, `@lhci/cli`, and MCP Inspector. `eslint-plugin-jsx-a11y` was evaluated but rejected because it crashes against the root toolchain's secure `minimatch` override; that override was not weakened. Versions are locked in the root or the owning package lockfiles.
