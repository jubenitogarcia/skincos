...
Target flow: local Website/Ponto harness with synthetic local-auth data -> the owning surface's shell -> keyboard navigation across the primary controls. The pilot does not submit a form or mutate data. It records axe JSON, Lighthouse HTML/JSON, failure-only traces/screenshots/video and native visual snapshots across four Chromium viewports.
