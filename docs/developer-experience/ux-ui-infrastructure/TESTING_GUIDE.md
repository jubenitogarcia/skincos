...
`npm run test:components` runs the Website Testing Library suite. `npm run test:e2e`, `test:e2e:ui`, and `test:e2e:headed` use the Website Playwright setup. `npm run test:a11y` records axe JSON; `npm run test:visual` uses native snapshots; `npm run audit:lighthouse` produces HTML/JSON; `npm run audit:bundle` reuses the Website bundle analyzer; `npm run audit:ui:full` starts and stops the local Website/Ponto harness and executes the pilot chain.
