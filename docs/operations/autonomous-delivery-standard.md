...
writes the private `/etc/skincos/global-coordination/native-runtime.env`
atomically and emits metadata only. Orb-specific release custody, workflow
promotion and provider readback are implemented in the independent Orb
repository; no Orb helper or credential path is retained in SKINCOS. No
workflow copies a secret to the checkout, Windows, an artifact, a comment, or
a log.
