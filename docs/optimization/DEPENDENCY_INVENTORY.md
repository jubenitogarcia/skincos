...
- Node packages (package.json) in:
  ...
  - CRM app (`/comprehensive-crm-so/package.json`)
  - WhatsApp Official Module (`/whatsapp/official-module/package.json`)
  - WhatsApp Library fork (`/whatsapp/official/package.json`)
  - Agent Zero monorepo root (`/agent-zero-module/package.json`)
  - Agent Zero shared configs (`/agent-zero-module/packages/shared-configs/package.json`)
  - WhatsApp backup stub (`/whatsapp/backup/package.json`)
- Python packages:
  - Agent Zero requirements (`/agent-zero-module/requirements.txt` and `requirements.unified.txt`)
...
- express ^4.21.1 (API dev server)
- axios ^1.7.7
- express-session ^1.18.2, cors ^2.8.5
- http-proxy-middleware ^3.0.3
- openid-client ^6.8.0, passport ^0.7.0, passport-oauth2 ^1.8.0
- pg ^8.16.3, connect-pg-simple ^10.0.0
- qrcode ^1.5.4
- memoizee ^0.4.17
...
Notes:
- Uses express v4.x (diverges from root/agent-zero express v5).
...
### Agent Zero (`/agent-zero-module/requirements.txt`)
...
### Agent Zero (`/agent-zero-module/requirements.unified.txt`)
- Similar set; includes commentary about merges with BroadHub.
...
- express versions:
  - v5.1.0: root, agent-zero, whatsapp/backup
  - v4.21.1: CRM
