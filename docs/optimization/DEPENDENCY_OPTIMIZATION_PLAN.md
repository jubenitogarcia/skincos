...
References:
- Inventory: `docs/optimization/DEPENDENCY_INVENTORY.md`
- Baseline script: `docs/performance/quick-baseline.sh`
...
- Node:
  - Fix body-parser version in `agent-zero-module/package.json` from ^2.2.0 to ^1.20.3.
...
- Target standard Express 4.x for all dev servers initially to reduce coordination risk (or 5.x if all features are compatible). Suggested path:
  ...
  - Downgrade root/agent-zero/whatsapp-backup from 5.1.0 → 4.21.1 where used for dev servers.
...
- Unit tests: expand orchestrator tests under `comprehensive-crm-so/src/services/__tests__/`.
- Smoke: run WA module, API, and Vite; verify /health and orchestrator flows.
- Performance: run `docs/performance/quick-baseline.sh` before/after each phase, store results in `docs/performance/baseline-<date>.md`.
