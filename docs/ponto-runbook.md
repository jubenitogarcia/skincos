...
1. Registrar o SHA imutável e verificar no host real o `crm.service`, listener, proxy e rotas/consumidores do Express, sem inferir estado live a partir do repositório. Capturar fora do Git o snapshot privado `ponto_store.v2.json` e a auditoria legada separada, com checksum; executar o preflight read-only de `docs/ponto-migration.md` contra o D1 de staging allowlisted antes de qualquer plano de importação.
...
não substitui o coordenador nem autoriza produção.
