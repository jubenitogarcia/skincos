...
export type FinanceRecurrence = { id: string; kind: 'payable' | 'receivable'; frequency: 'monthly'; description: string; amount_minor: number; currency: string; competence_day: number; due_day: number; starts_on: string; ends_on?: string | null; next_due_date: string; active: number; category_id?: string | null; payee_id?: string | null; cost_center_id?: string | null; notes?: string | null }
...
export const financeApi = {
  ...
  recurrences: (scopeId: string, options: { page?: number; limit?: number } = {}) => request<{ ok: true; page: number; limit: number; recurrences: FinanceRecurrence[] }>(`/recurrences?${scopedQuery(scopeId, options)}`),
  createRecurrence: (scopeId: string, body: Record<string, unknown>, idempotencyKey?: string) => request<any>(`/recurrences?${scopedQuery(scopeId)}`, { method: 'POST', headers: idempotencyHeaders(idempotencyKey), body: JSON.stringify(body) }),
  materializeRecurrence: (scopeId: string, recurrenceId: string, throughDate: string, idempotencyKey?: string) => request<any>(`/recurrences/${encodeURIComponent(recurrenceId)}/materialize?${scopedQuery(scopeId)}`, { method: 'POST', headers: idempotencyHeaders(idempotencyKey), body: JSON.stringify({ throughDate }) }),
  ...
  cancelObligation: (scopeId: string, obligationId: string, reason: string, idempotencyKey?: string) => request<any>(`/obligations/${encodeURIComponent(obligationId)}/cancel?${scopedQuery(scopeId)}`, { method: 'POST', headers: idempotencyHeaders(idempotencyKey), body: JSON.stringify({ reason }) }),
