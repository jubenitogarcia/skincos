...
# Diretórios
CRM_DIR="$(cd "$(dirname "$0")" && pwd)"                # frontend/
ROOT_DIR="$CRM_DIR"
...
resolve_skincos_root() {
  ...
  for candidate in "$base" "$base/.." "$base/../.." "$base/../../.."; do
    if [[ -d "$candidate/backend/apps/crm-api" && -d "$candidate/frontend" && -f "$candidate/frontend/package.json" ]]; then
      (cd "$candidate" && pwd)
      return 0
    fi
  done
...
API_DIR="$SKINCOS_ROOT/backend/apps/crm-api"
...
# API deps are isolated in backend/apps/crm-api for faster installs
if [[ $START_API -eq 1 ]]; then
    if [[ ! -f "$API_DIR/package.json" ]]; then
      ...
      if [[ $api_needs_install -eq 1 ]]; then
        echo "[restart_crm] Instalando dependências da API (isoladas em backend/apps/crm-api)..."
        (
