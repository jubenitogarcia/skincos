...
function validRows() {
  return [
    { event_order: 1, event_id: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', identity_id: A, unit_slug: 'jardins', revision: 1, operation: 'upsert', occurred_at: '2026-09-08T12:00:00.000000Z' },
    { event_order: 2, event_id: 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb', identity_id: A, unit_slug: 'jardins', revision: 2, operation: 'revoke', occurred_at: '2026-09-08T12:01:00.000000Z' },
    { event_order: 3, event_id: 'cccccccc-cccc-4ccc-8ccc-cccccccccccc', identity_id: B, unit_slug: 'pinheiros', revision: 1, operation: 'upsert', occurred_at: '2026-09-08T12:02:00.000000Z' },
  ]
}
...
test('runs bounded outbox pages, accepts receipts, resumes through an opaque checkpoint and completes', async () => {
  ...
  const summary = await runner.run({ intent: ATENDIMENTO_CRM_PROJECTION_DELTA_RUN_INTENT })
...
test('rejects revision regression before signing or delivery', async () => {
  ...
  await assert.rejects(() => runner.run({ intent: ATENDIMENTO_CRM_PROJECTION_DELTA_RUN_INTENT }), /REVISION_REGRESSION/)
