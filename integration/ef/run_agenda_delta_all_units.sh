...
# Load local sync secrets from a user-level file (outside repo) when available.
...
# Load login credentials from a user-level file (outside repo) when available.
...
if [ -n "${EF_AGENDA_SYNC_URL:-}" ] && [ -z "${EF_AGENDA_SYNC_TOKEN:-}" ]; then
  echo "WARNING: EF_AGENDA_SYNC_URL is set but EF_AGENDA_SYNC_TOKEN is missing." >&2
  echo "WARNING: Sync endpoint call may fail. Configure token in $sync_env_file or environment." >&2
fi
