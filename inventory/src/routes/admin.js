...
export async function handleAdminRoutes({
  ...
  if ((url.pathname === '/admin/onboarding' || url.pathname === '/admin/team') && request.method === 'POST') {
    try {
      ...
      if (url.pathname === '/admin/team' && !TEAM_ADMIN_ROLES.includes(normalizeRole(auth?.user?.role))) {
  ...
  if (url.pathname === '/admin/team' && request.method === 'GET') {
    try {
      const rows = await env.DB.prepare(`SELECT o.*, t.schedule_professional_id, t.schedule_status, t.schedule_role, t.schedule_shift, t.schedule_nickname, t.schedule_instagram, t.schedule_color, t.units_json AS schedule_units_json
        ...
        WHERE o.account_status IN ('INVITED', 'ACTIVE') ORDER BY o.created_at DESC LIMIT 500`).all();
      const visible = (rows?.results || []).filter((row) => teamUnitsVisible(auth, row.units_json));
      ...
      return withCORS(JSON.stringify({ success: true, data: visible.map((row) => publicTeamMember(row, linksByEmployee.get(String(row.workforce_employee_id || '')) || [])), activeOnly: true }), { status: 200 }, appOrigin);
    } catch {
