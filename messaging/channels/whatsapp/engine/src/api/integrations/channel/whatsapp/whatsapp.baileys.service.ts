...
const ARCHIVED_SYSTEM_LABEL = '__archived__';
const ARCHIVED_SYSTEM_LABEL_ALIASES = new Set([ARCHIVED_SYSTEM_LABEL, 'archived', 'arquivada']);
...
export class BaileysStartupService extends ChannelStartupService {
  ...
  private normalizeLabels(raw: unknown): string[] {
  ...
  private resolveArchivedFlag(raw: unknown): boolean | null {
  ...
  private mergeArchivedLabel(labelsRaw: unknown, archivedFlag: boolean | null): string[] | undefined {
    const labels = this.normalizeLabels(labelsRaw);
    if (archivedFlag === null && labels.length === 0) return undefined;

    const labelSet = new Set(labels);
    ARCHIVED_SYSTEM_LABEL_ALIASES.forEach((label) => labelSet.delete(label));
    if (archivedFlag === true) {
      labelSet.add(ARCHIVED_SYSTEM_LABEL);
    }

    return Array.from(labelSet);
  }

  private async persistArchivedLabel(remoteJid: string, archived: boolean): Promise<void> {
    const normalizedRemoteJid = createJid(remoteJid);
    ...
    const mergedLabels = this.mergeArchivedLabel(existing?.labels, archived) ?? [];

    if (existing?.id) {
      await this.prismaRepository.chat.update({
        where: { id: existing.id },
        data: { labels: mergedLabels, remoteJid: normalizedRemoteJid },
      });
      return;
    }
  ...
  private async createClient(number?: string): Promise<WASocket> {
    ...
    const disableHistoryByEnv =
      process.env.CONFIG_DISABLE_FULL_HISTORY === 'true' || process.env.DATABASE_SAVE_DATA_HISTORIC === 'false';
    const localSyncFullHistory =
      typeof this.localSettings?.syncFullHistory === 'boolean' ? this.localSettings.syncFullHistory : true;
    const ignoreAllJids = process.env.CONFIG_IGNORE_ALL_JIDS === 'true';
    ...
    const socketConfig: UserFacingSocketConfig = {
      ...
      // Keep history sync enabled by default so newly connected channels can backfill chats/messages.
      syncFullHistory: !disableHistoryByEnv && localSyncFullHistory,
      shouldSyncHistoryMessage: () => !disableHistoryByEnv,

      // Do not ignore all JIDs by default (that prevents chat/message persistence).
      shouldIgnoreJid: () => ignoreAllJids,
    ...
    // Force disable history sync if environment explicitly disables it.
    if (disableHistoryByEnv) {
  ...
  private readonly chatHandle = {
    'chats.upsert': async (chats: Chat[]) => {
      ...
      const chatsToInsert = chats
        ...
        .map((chat) => {
          const archivedFlag = this.resolveArchivedFlag((chat as any)?.archived ?? (chat as any)?.archive);
          const labels = this.mergeArchivedLabel((chat as any)?.labels, archivedFlag);
          return {
            remoteJid: chat.id,
            instanceId: this.instanceId,
            name: chat.name,
    ...
    'chats.update': async (
      ...
      for (const chat of chats) {
        ...
        if (typeof chat?.name === 'string' && chat.name.trim().length > 0) {
          updateData.name = chat.name.trim();
        }
        if (chat?.unreadCount !== undefined && Number.isFinite(Number(chat.unreadCount))) {
          updateData.unreadMessages = Number(chat.unreadCount);
        }
        if (labels) {
        ...
        if (typeof archivedFlag === 'boolean') {
          await this.persistArchivedLabel(chat.id, archivedFlag);
        }
      }
    },
    ...
    'messaging-history.set': async ({
      ...
      try {
        ...
        for (const chat of chats) {
          const archivedFlag = this.resolveArchivedFlag((chat as any)?.archived ?? (chat as any)?.archive);
          const labels = this.mergeArchivedLabel((chat as any)?.labels, archivedFlag);
          const baseData: { remoteJid: string; name?: string; unreadMessages?: number; labels?: string[] } = {
            remoteJid: chat.id,
            unreadMessages: chat.unreadCount !== undefined ? chat.unreadCount : 0,
          };
          if (typeof chat?.name === 'string' && chat.name.trim().length > 0) {
            baseData.name = chat.name.trim();
          }
          if (labels) {
            baseData.labels = labels;
          }
        ...
        if (this.configService.get<Database>('DATABASE').SAVE_DATA.HISTORIC) {
          ...
          for (const chat of chatsToUpdate) {
            ...
            if (typeof chat.archivedFlag === 'boolean') {
              await this.persistArchivedLabel(chat.remoteJid, chat.archivedFlag);
            }
          }
        }
  ...
  public async getLastMessage(number: string) {
    const normalizedRemoteJid = createJid(number);
    const message = await this.prismaRepository.message.findFirst({
      where: {
        instanceId: this.instance.id,
        AND: [{ key: { path: ['remoteJid'], equals: normalizedRemoteJid } }],
      },
  ...
  public async archiveChat(data: ArchiveChatDto) {
    try {
      ...
      let number = data.chat ? createJid(data.chat) : undefined;

      if (!last_message && number) {
        last_message = await this.getLastMessage(number);
      }
      if (last_message?.key?.remoteJid) {
        number = createJid(last_message.key.remoteJid);
      }
      if (last_message && !last_message?.messageTimestamp) {
      ...
      await this.client.chatModify({ archive: data.archive, lastMessages: [last_message] }, number);
      await this.persistArchivedLabel(number, Boolean(data.archive));

      return { chatId: number, archived: Boolean(data.archive) };
    } catch (error) {
