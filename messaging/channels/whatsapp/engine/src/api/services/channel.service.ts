...
export class ChannelStartupService {
  ...
  private parseChatLabels(raw: unknown): string[] {
  ...
  private isArchivedByLabels(raw: unknown): boolean {
  ...
  public async fetchChats(query: any) {
    ...
    const archivedLabelFilter =
      archivedRequested === true
        ...
        : archivedRequested === false
          ? Prisma.sql`AND NOT COALESCE("Chat"."labels" @> '["__archived__"]'::jsonb, false)`
          : Prisma.sql``;
    ...
    if (results && isArray(results) && results.length > 0) {
      const mappedResults = results.map((contact) => {
        const labels = this.parseChatLabels(contact.labels);
        const isArchived = this.isArchivedByLabels(labels);
