...
class Client extends EventEmitter {
    ...
    /**
     * Internal: determine if the error is a transient puppeteer page/frame context error
     * that can be retried after waiting for the page to stabilize.
     * @param {any} err
     * @returns {boolean}
     */
    _isTransientPageError(err) {
        ...
        return [
            ...
            'target closed'
        ].some(snippet => msg.includes(snippet));
    }

    /**
     * Internal: wait for the WhatsApp Web page context to be ready/stable.
     * Avoids throwing if the page is navigating or reloading by using waitForFunction
     * that binds to the current main frame automatically.
     * @param {number} timeoutMs
     */
    async _waitForPageReady(timeoutMs = 15000) {
        ...
        // Wait for WA objects to be present
        try {
            await this.pupPage.waitForFunction(
                'window && window.Debug && window.Debug.VERSION && window.Store && window.WWebJS && window.Store.Chat',
                { timeout: timeoutMs }
            );
        } catch (_) { }
        // Small settle delay
    ...
    /**
     * Internal: evaluate wrapper with retry on transient page/frame errors.
     * @template T
     * @param {() => Promise<T>} evalFn
     * @param {number} retries
     * @returns {Promise<T>}
     */
    async _evaluateWithRetry(evalFn, retries = 2) {
        let lastErr;
        for (let attempt = 0; attempt <= retries; attempt++) {
            try {
                ...
                lastErr = err;
                if (this._isTransientPageError(err) && attempt < retries) {
                    await this._waitForPageReady(15000);
                    continue;
                }
                throw err;
            }
        }
        // Should not get here, but in case return throw last error
        throw lastErr;
    }
    /**
     * Injection logic
     ...
     */
    async getChats() {
        const chats = await this._evaluateWithRetry(async () => {
    ...
    async getChatById(chatId) {
        const chat = await this._evaluateWithRetry(async () => {
            return await this.pupPage.evaluate(async chatId => {
     ...
     */
    async getContacts() {
        let contacts = await this._evaluateWithRetry(async () => {
            return await this.pupPage.evaluate(() => {
    ...
    async getMessageById(messageId) {
        const msg = await this._evaluateWithRetry(async () => {
