...
const SERVICE_PORT = Number(process.env.WHATSAPP_PORT || process.env.PORT || 3001);

function isReadyStatus(rawStatus) {
    return rawStatus === 'ready' || rawStatus === 'connected';
}

function canonicalizeStatus(rawStatus) {
    if (rawStatus === 'connected') return 'ready';
    return rawStatus || 'disconnected';
}

function buildStatusMessage({ ready, qrRequired, status }) {
    if (ready) return 'Bot está pronto';
    if (qrRequired) return 'Escaneie o QR Code';
    if (status === 'authenticated') return 'Autenticado. Finalizando conexão…';
    if (status === 'qr_received') return 'QR Code recebido. Escaneie para conectar.';
    if (status === 'disconnected') return 'Bot desconectado';
    if (status === 'recovering') return 'Recuperando conexão…';
    if (status === 'recovery_failed') return 'Falha de recuperação';
    return 'Bot conectando...';
}

function getChannel1Snapshot() {
    try {
        if (!MULTI_CHANNEL_MODE || !channelManagementAPI || !channelManagementAPI.channelManager) return null;
        const channelStatus = channelManagementAPI.channelManager.getChannelStatus('1');
        if (!channelStatus || channelStatus.success !== true) return null;
        return {
            rawStatus: channelStatus.status,
            qr: channelStatus.qrCode || null,
            clientInfo: channelStatus.clientInfo || null
        };
    } catch {
        return null;
    }
...
function buildCompatStatusPayload(source = 'legacy') {
    const channel1 = getChannel1Snapshot();
    const rawStatus = channel1 ? channel1.rawStatus : clientStatus;
    const qr = channel1 ? channel1.qr : currentQR;
    const info = channel1 ? channel1.clientInfo : clientInfo;

    const ready = isReadyStatus(rawStatus);
    const status = canonicalizeStatus(rawStatus);
    const qrRequired = !!qr && !ready;
    ...
    return {
        ...
        rawStatus,
        connState: null,
        message: buildStatusMessage({ ready, qrRequired, status: rawStatus }),
        qrRequired,
        didAuth: rawStatus === 'authenticated' || ready,
        instancePort: SERVICE_PORT,
        user: info?.pushname || null,
...
app.get('/status', (req, res) => {
    const payload = buildCompatStatusPayload(MULTI_CHANNEL_MODE ? 'channel-1' : 'legacy');
    res.json(payload);
});

app.get('/v1/status', (req, res) => {
    const payload = buildCompatStatusPayload(MULTI_CHANNEL_MODE ? 'channel-1' : 'legacy');
    // Keep backward compatibility: v1/status historically omitted some metadata
    const { module, version, ...rest } = payload;
    res.json(rest);
});
...
app.get('/qr', (req, res) => {
    ...
    if (qr) {
        return res.json({
            success: true,
            qr,
            status: ready ? 'ready' : (rawStatus || 'qr_available'),
            message: 'QR Code disponível',
            timestamp: new Date().toISOString(),
            source: channel1 ? 'channel-1' : 'legacy'
        });
    }
    ...
    return res.json({
        success: false,
        qr: null,
        status: ready ? 'ready' : (rawStatus || 'disconnected'),
        message: ready ? 'Bot já conectado' : 'QR Code não disponível ainda',
        timestamp: new Date().toISOString(),
        source: channel1 ? 'channel-1' : 'legacy'
    });
});
...
function getChannel1Instance() {
    try {
        if (!MULTI_CHANNEL_MODE || !channelManagementAPI || !channelManagementAPI.channelManager) return null;
        return channelManagementAPI.channelManager.getChannelInstance('1') || null;
    } catch {
        return null;
    }
}
...
app.post('/send', async (req, res) => {
    try {
        ...
        // Prefer multi-channel Channel 1 (internal, no HTTP/auth)
        const ch1 = getChannel1Instance();
        if (ch1 && ch1.client) {
            const ch1Ready = isReadyStatus(ch1.status);
            if (!ch1Ready) {
                return res.status(503).json({
                    success: false,
                    error: 'Channel 1 not ready',
                    status: ch1.status,
                    hint: 'Activate channel 1 via POST /api/channel-manager/channels/1/activate',
                    source: 'channel-1'
                });
            }

            const chatId = phone.includes('@c.us') ? phone : `${phone}@c.us`;
            const result = await ch1.client.sendMessage(chatId, message);
            return res.json({
                success: true,
...
app.get('/chats', async (req, res) => {
    try {
        ...
        if (ch1 && ch1.client) {
            ...
            const chats = await ch1.client.getChats();
            const formattedChats = chats.map(chat => ({
                id: chat.id._serialized,
                name: chat.name,
                isGroup: chat.isGroup,
...
// Start the server
app.listen(SERVICE_PORT, '0.0.0.0', async () => {
    console.log(`🚀 WhatsApp Official Module server running on port ${SERVICE_PORT}`);
    console.log(`📱 Web interface: http://localhost:${SERVICE_PORT}`);
    ...
    try {
        if (MULTI_CHANNEL_MODE) {
            ...
            channelManagementAPI = new ChannelManagementAPI({
                ...
                // Default to the same writable base used by LocalAuth (set by scripts via WHATSAPP_DATA_PATH),
                // so we don't create sessions/config inside the repository tree.
                basePath: path.dirname(DATA_PATH),
            ...
            console.log(`📊 Channel Manager: http://localhost:${SERVICE_PORT}/api/channel-manager/system/status`);
