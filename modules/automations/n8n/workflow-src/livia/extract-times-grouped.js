...
function getWaitAt(hour, minute) {
  if (typeof $now !== "undefined" && $now && typeof $now.set === "function") {
    return $now.set({
      hour,
      minute,
      second: 0,
...
for (const file of unpublished) {
  ...
  const waitAt = getWaitAt(hour, minute);
  if (waitAt <= $now) continue;

  const waitUntil = typeof waitAt.toISO === "function" ? waitAt.toISO() : "";
  ...
  if (!groupsByKey.has(groupKey)) {
    ...
    groupsByKey.set(groupKey, record);
    scheduledGroups.push(record);
  }

  groupsByKey.get(groupKey).items.push(file);
}

scheduledGroups.sort((left, right) => {
  if (left.waitUntil !== right.waitUntil) return left.waitUntil.localeCompare(right.waitUntil);
  return left.postPrefix.localeCompare(right.postPrefix);
});

return scheduledGroups.map((group, groupOrder) => {
