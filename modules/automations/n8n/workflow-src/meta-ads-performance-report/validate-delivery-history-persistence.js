...
const preparedItems = $('Prepare Delivery History Persistence').all();
...
return $input.all().map((item) => {
  ...
  if (!ok && !endpointUnavailable) {
    throw new Error(JSON.stringify({
      type: 'delivery_history_persistence_error',
  ...
  return {
    json: {
      ...
      persistence_status: endpointUnavailable
        ? 'delivery_history_endpoint_unavailable'
