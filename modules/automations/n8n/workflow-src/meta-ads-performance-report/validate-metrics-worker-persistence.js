...
const preparedItems = $('Prepare Metrics Worker Persistence').all();
...
return $input.all().map((item) => {
  ...
  if (!ok) {
    throw new Error(JSON.stringify({
      type: 'metrics_worker_persistence_error',
  ...
  return {
    json: {
      ...
      worker_persistence: {
        ...
        idempotent_replay: body?.idempotentReplay === true,
        in_progress: body?.inProgress === true,
        request_id: safeString(body?.requestId || body?.results?.request_id),
