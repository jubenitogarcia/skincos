...
const ADVANTAGE_PLUS_ALWAYS_ELIGIBLE_FEATURES = [
  'add_text_overlay',
  'image_touchups',
  'text_optimizations',
  'inline_comment',
...
function buildAdvantagePlusRequest({ apiVersion, siteLinks, musicEligible, pacEligible, landingPageUrl }) {
  ...
  const eligibleFeatures = [...ADVANTAGE_PLUS_ALWAYS_ELIGIBLE_FEATURES];
  const skippedFeatures = [];
  const skipReasons = {};
  for (const feature of ADVANTAGE_PLUS_ALWAYS_ELIGIBLE_FEATURES) {
  ...
  return {
    ...
    desiredFeatures: [...ADVANTAGE_PLUS_ALWAYS_ELIGIBLE_FEATURES, ...ADVANTAGE_PLUS_CONDITIONAL_FEATURES],
    ...
    creativeSourcingSpec: {
      source_url: landingPageUrl,
      ...(siteLinksEligible
...
for (const entry of jobEntries) {
  ...
  for (const destinationMeta of resolvedDestinations) {
    ...
    const ctaTypes = [DEFAULT_CTA_TYPE];
    ...
    const primaryLinkUrl = landingPage.url;
    ...
    outputs.push({
      json: {
        ...
        advantage_plus_request: {
          desired_features: deepClone(advantagePlusRequest.desiredFeatures),
          requested_features: deepClone(advantagePlusRequest.requestedFeatures),
          eligible_features: deepClone(advantagePlusRequest.eligibleFeatures),
          skipped_features: deepClone(advantagePlusRequest.skippedFeatures),
          skip_reasons: deepClone(advantagePlusRequest.skipReasons),
        ...
        advantage_plus_eligible_features: deepClone(advantagePlusRequest.eligibleFeatures),
        advantage_plus_skipped_features: deepClone(advantagePlusRequest.skippedFeatures),
        advantage_plus_skip_reasons: deepClone(advantagePlusRequest.skipReasons),
