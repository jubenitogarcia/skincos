...
import { DEFAULT_ATENDIMENTO_FILTERS, formatCurrencyBRL, formatNumberBR, parseBrazilCurrency } from '@/atendimentoDomain'
import {
  ...
  return (
    <div className="atendimento-surface flex min-h-full flex-col gap-5 px-3 pb-6 pt-3 text-white sm:px-6">
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <Metric label="Faturamento Atendimento" value={formatCurrencyBRL(commercial?.summary?.totalValue || 0)} detail="Base transacional do Atendimento" />
      ...
      <Card className={`${panelClass}`}>
        <CardHeader className="flex-row items-start justify-between gap-4">
          <div>
            ...
            <div className="mt-1 text-xs text-slate-500">Meta por unidade e mês usada pelo Atendimento e demais módulos do CRM.</div>
          </div>
        </CardHeader>
