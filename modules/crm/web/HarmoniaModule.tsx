...
export function HarmoniaModule({ mode = 'full', showHeader = true, showChannels = true }: HarmoniaModuleProps) {
  const [tab, setTab] = React.useState<'canais' | 'overview' | 'conversations' | 'tasks'>(() => {
    try {
      const stored = localStorage.getItem('harmonia.ui.tab')
      if (stored === 'overview' || stored === 'conversations' || stored === 'tasks' || stored === 'canais') {
        return stored
      }
    } catch { /* ignore */ }
    return 'canais'
      ...
      <div className="p-6 space-y-4">
        ...
        {!dbConfigured ? (
          <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-3 text-sm text-amber-100">
            O módulo de leads está ativo, mas sem persistência. Configure `DATABASE_URL` no `modules/crm/api` para liberar conversas,
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <Tabs value={tab} onValueChange={(v) => setTab(v as any)}>
        <TabsList>
          {showChannels ? <TabsTrigger value="canais">Canais</TabsTrigger> : null}
        ...
        {showChannels ? (
          ...
          <Card className="bg-white/[0.06] border-white/10">
            <CardHeader>
              <CardTitle className="text-white">Central de Conversas</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
