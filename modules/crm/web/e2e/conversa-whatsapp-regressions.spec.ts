...
async function openConversa(page: any) {
  await page.goto('/?module=conversa')
  await page.evaluate(() => {
    localStorage.setItem('app.activeModule', 'conversa')
  })
  await page.reload()
  await authenticateIfNeeded(page)
  const heading = page.getByRole('heading', { name: 'Conversa' }).first()
  if (!(await heading.isVisible().catch(() => false))) {
    const conversaNav = page.getByRole('button', { name: 'Conversa' }).first()
...
test.describe('conversa whatsapp regressions', () => {
