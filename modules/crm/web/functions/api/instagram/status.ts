import { isLocalDevAuthBypassEnabled, requireCrmUser } from '../../_lib/crmAuth'
import { getShareBucket } from '../../_lib/r2'
import { graphGet } from '../../_lib/instagramGraph'
import { readConnectionDecrypted } from '../../_lib/instagramStore'
import { getIntegrationsEncryptionSecret, integrationsEncryptionSecretRequired, isMissingIntegrationsEncryptionSecretError } from '../../_lib/integrationsEncryption'

const json = (status: number, body: any) =>
  new Response(JSON.stringify(body), {
    status,
    headers: { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' },
  })

export async function onRequestGet(context: any): Promise<Response> {
  const userOrRes = await requireCrmUser(context)
  if (userOrRes instanceof Response) return userOrRes

  const localStub = isLocalDevAuthBypassEnabled(context)
  const bucket = getShareBucket(context)
  if (!bucket) {
    if (localStub) {
      return json(200, { ok: true, connected: false, localStub: true, reason: 'SHARE_BUCKET_NOT_CONFIGURED' })
    }
    return json(503, { ok: false, error: 'SHARE_BUCKET not configured' })
  }

  const secret = getIntegrationsEncryptionSecret(context)
  if (integrationsEncryptionSecretRequired(context) && !secret) {
    if (localStub) {
      return json(200, { ok: true, connected: false, localStub: true, reason: 'INTEGRATIONS_ENCRYPTION_SECRET_REQUIRED' })
    }
    return json(503, { ok: false, error: 'INTEGRATIONS_ENCRYPTION_SECRET_REQUIRED' })
  }

  let conn: any = null
  try {
    conn = await readConnectionDecrypted(bucket, userOrRes.id, secret)
  } catch (e: any) {
    if (!secret && isMissingIntegrationsEncryptionSecretError(e)) {
      if (localStub) {
        return json(200, { ok: true, connected: false, localStub: true, reason: 'INTEGRATIONS_ENCRYPTION_SECRET_REQUIRED' })
      }
      return json(503, { ok: false, error: 'INTEGRATIONS_ENCRYPTION_SECRET_REQUIRED' })
    }
    return json(500, { ok: false, error: 'INTERNAL' })
  }
  if (!conn) return json(200, { ok: true, connected: false })

  let username: string | undefined
  try {
    const info = await graphGet<{ username?: string }>(conn.igBusinessAccountId, { fields: 'username' }, conn.accessToken)
    username = info?.username
  } catch {
    username = undefined
  }

  return json(200, {
    ok: true,
    connected: true,
    businessAccountId: conn.igBusinessAccountId,
    pageId: conn.pageId,
    username,
    tokenType: conn.tokenType,
    updatedAt: conn.updatedAt,
  })
}
