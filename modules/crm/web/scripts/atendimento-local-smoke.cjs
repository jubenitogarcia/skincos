...
const URL = process.env.CRM_URL || 'http://127.0.0.1:5173/?module=atendimento'
...
async function main() {
  ...
  const screenshotPath = path.join(ARTIFACT_DIR, `atendimento-local-${stamp}.png`)
  let browser = null
  try {
    ...
    await expectVisible(page.getByRole('heading', { name: 'Atendimento' }), 'Atendimento heading')
    ...
    const globalError = await page.getByText(/DATABASE_URL_not_configured|ATENDIMENTO_API_TARGET|UPSTREAM_UNREACHABLE/).first().isVisible().catch(() => false)
    if (globalError) {
      throw new Error('Atendimento abriu, mas a API local não está configurada para carregar dados. Configure DATABASE_URL e revise o target local.')
    }
    ...
    if (consoleErrors.length) {
      console.log('[atendimento-local-smoke] Console errors (first 5):')
      for (const error of consoleErrors.slice(0, 5)) console.log('  -', error)
    }

    console.log(`[atendimento-local-smoke] OK: ${URL}`)
  } finally {
...
main().catch((err) => {
  console.error('[atendimento-local-smoke] FAIL:', err?.message || err)
  process.exitCode = 1
})
