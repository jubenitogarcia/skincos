...
Copie `.env.example` para seu ambiente do n8n e ajuste:
- `N8N_DEFAULT_UNIT_SLUG`
- `N8N_DEFAULT_UNIT_NAME`
- `N8N_UNIT_NAME_MAP`
- `N8N_HANDOFF_NOTIFY_NUMBER`
- `GOOGLE_CALENDAR_ID` (opcional enquanto o agendamento com Google Calendar nao estiver sendo exigido no runtime live)
...
2. **Google Calendar (OAuth2)**
   ...
   - Hoje isso pode permanecer em branco se o fluxo de agendamento com Google Calendar ainda nao estiver sendo ativado no runtime live.
...
## Teste mínimo
1) Disparar `POST` no webhook com `sample_payloads/evolution_inbound_message.json`
```bash
curl -X POST http://localhost:5678/webhook-test/wa/inbound/evolution \
  -H 'Content-Type: application/json' \
  -d @sample_payloads/evolution_inbound_message.json
```
