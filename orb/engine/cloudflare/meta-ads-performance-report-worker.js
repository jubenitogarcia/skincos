export default {
  async fetch(request, env) {
    ...
    if (request.method === 'POST' && url.pathname === '/ingest/meta-ads-report-history') {
      return handleReportHistoryIngestion(request, env);
    }
...
async function handleReportHistoryIngestion(request, env) {
  const requestId = crypto.randomUUID();
  const startedAt = new Date().toISOString();
  const authResult = await authorizeRequest(request, env);

  if (!authResult.ok) {
    log(env, 'warn', 'report_history_auth_failed', {
      requestId,
      reason: authResult.reason,
    });
  ...
  const validation = validateReportHistoryBody(body);
  if (!validation.ok) {
    return jsonResponse(400, {
      ok: false,
      error: 'invalid_body',
      message: 'Invalid report history ingestion payload.',
      issues: validation.issues,
      requestId,
    });
  }
  ...
  const run = isObject(body.run) ? body.run : {};
  const groupedSnapshots = asArray(body.grouped_snapshots);
  const subjectiveReviews = asArray(body.subjective_reviews);
  const consolidatedReport = isObject(body.consolidated_report) ? body.consolidated_report : null;
  const deliveryAudit = isObject(body.delivery_audit) ? body.delivery_audit : null;
  const idempotencyKey = safeString(
    request.headers.get('Idempotency-Key') ||
    `${safeString(run.run_id)}:${safeString(run.report_key)}:${safeString(run.ingestion_kind || 'report_history')}`
  );
  ...
  if (existingRun?.status === 'completed' && existingRun.summary_json) {
    return jsonResponse(200, {
      ...
      idempotentReplay: true,
      requestId,
      results: safeJsonParse(existingRun.summary_json, {}),
    });
  }

  if (existingRun?.status === 'in_progress' && !isStaleRun(existingRun.updated_at)) {
    return jsonResponse(202, {
      ok: true,
      inProgress: true,
      requestId,
  ...
  const requestMeta = {
    requestId,
    reportKey: safeString(run.report_key),
    ingestionKind: safeString(run.ingestion_kind),
    reportDate: safeString(run.report_date),
    accountId: safeString(run.account_id),
    groupedSnapshots: groupedSnapshots.length,
    subjectiveReviews: subjectiveReviews.length,
    consolidatedReport: consolidatedReport ? 1 : 0,
    deliveryAudit: deliveryAudit ? 1 : 0,
    idempotencyKey,
  };
  ...
  const runRowBase = {
    run_id: safeString(run.run_id || idempotencyKey),
    report_key: safeString(run.report_key),
    ingestion_kind: safeString(run.ingestion_kind || 'report_history'),
    report_date: safeString(run.report_date),
    account_id: safeString(run.account_id),
    account_name: safeString(run.account_name),
    idempotency_key: idempotencyKey,
    created_at: createdAt,
  };
  const progress = {
    ...
    attempt_count: attemptCount,
    last_request_id: requestId,
    grouped_snapshots_upserted: 0,
    subjective_reviews_upserted: 0,
    consolidated_reports_upserted: 0,
    delivery_audits_upserted: 0,
    last_error: '',
    summary_json: JSON.stringify({
      status: 'in_progress',
      phase: 'accepted',
  ...
  async function syncRunProgress(overrides = {}) {
    Object.assign(progress, overrides);
    await upsertReportIngestionRun(env, {
      ...runRowBase,
      ...progress,
    });
  }
  ...
  try {
    ...
    if (groupedSnapshots.length) {
      await syncRunProgress({
        phase: 'd1_group_snapshots_started',
        updated_at: new Date().toISOString(),
      });
      await runStatementsInChunks(env.META_ADS_DB, groupedSnapshots.map((entry) => buildReportGroupStatement(env, entry)));
      await syncRunProgress({
        phase: 'd1_group_snapshots_completed',
        last_successful_phase: 'd1_group_snapshots',
        grouped_snapshots_upserted: groupedSnapshots.length,
        updated_at: new Date().toISOString(),
      });
    }

    if (subjectiveReviews.length) {
      await syncRunProgress({
        phase: 'd1_subjective_reviews_started',
        updated_at: new Date().toISOString(),
      });
      await runStatementsInChunks(env.META_ADS_DB, subjectiveReviews.map((entry) => buildReportSubjectiveReviewStatement(env, entry)));
      await syncRunProgress({
        phase: 'd1_subjective_reviews_completed',
        last_successful_phase: 'd1_subjective_reviews',
        subjective_reviews_upserted: subjectiveReviews.length,
        updated_at: new Date().toISOString(),
      });
    }
    ...
    if (consolidatedReport) {
      await syncRunProgress({
        phase: 'd1_consolidated_report_started',
        updated_at: new Date().toISOString(),
      });
      await runStatementsInChunks(env.META_ADS_DB, [buildConsolidatedReportStatement(env, consolidatedReport)]);
      await syncRunProgress({
        phase: 'd1_consolidated_report_completed',
        last_successful_phase: 'd1_consolidated_report',
        consolidated_reports_upserted: 1,
        updated_at: new Date().toISOString(),
      });
    }
    ...
    if (deliveryAudit) {
      await syncRunProgress({
        phase: 'd1_delivery_audit_started',
        updated_at: new Date().toISOString(),
      });
      await runStatementsInChunks(env.META_ADS_DB, [buildReportDeliveryAuditStatement(env, deliveryAudit)]);
      await syncRunProgress({
        phase: 'd1_delivery_audit_completed',
        last_successful_phase: 'd1_delivery_audit',
        delivery_audits_upserted: 1,
        updated_at: new Date().toISOString(),
      });
    }
    ...
    const summary = {
      grouped_snapshots_upserted: groupedSnapshots.length,
      subjective_reviews_upserted: subjectiveReviews.length,
      consolidated_reports_upserted: consolidatedReport ? 1 : 0,
      delivery_audits_upserted: deliveryAudit ? 1 : 0,
      idempotency_key: idempotencyKey,
      request_id: requestId,
      report_key: safeString(run.report_key),
      ingestion_kind: safeString(run.ingestion_kind || 'report_history'),
    ...
    await syncRunProgress({
      ...
      phase: 'completed',
      last_successful_phase: 'completed',
      grouped_snapshots_upserted: summary.grouped_snapshots_upserted,
      subjective_reviews_upserted: summary.subjective_reviews_upserted,
      consolidated_reports_upserted: summary.consolidated_reports_upserted,
      delivery_audits_upserted: summary.delivery_audits_upserted,
      summary_json: JSON.stringify(summary),
      updated_at: new Date().toISOString(),
    });

    log(env, 'info', 'report_history_ingestion_completed', {
      ...requestMeta,
      ...summary,
    });
    ...
    try {
      await syncRunProgress({
        ...
        summary_json: JSON.stringify({
          ...
          phase: progress.phase,
          last_successful_phase: progress.last_successful_phase,
          grouped_snapshots_upserted: progress.grouped_snapshots_upserted,
          subjective_reviews_upserted: progress.subjective_reviews_upserted,
          consolidated_reports_upserted: progress.consolidated_reports_upserted,
          delivery_audits_upserted: progress.delivery_audits_upserted,
          attempt_count: progress.attempt_count,
        }),
        updated_at: new Date().toISOString(),
      });
    } catch (secondaryError) {
      log(env, 'error', 'report_history_run_update_failed', {
        requestId,
        error: String(secondaryError?.message || secondaryError),
      });
    }

    log(env, 'error', 'report_history_ingestion_failed', {
      ...requestMeta,
      error: message,
    });
    ...
    return jsonResponse(500, {
      ok: false,
      error: 'report_history_ingestion_failed',
      message,
      requestId,
      phase: safeString(progress.phase),
      lastSuccessfulPhase: safeString(progress.last_successful_phase),
...
function buildContractResponse(env) {
  return {
    ...
    contract: {
      ...
      report_history: {
        method: 'POST',
        endpoint: '/ingest/meta-ads-report-history',
        required_headers: [
          'Content-Type: application/json',
          'Idempotency-Key',
          safeString(env.WORKER_AUTH_HEADER_NAME || 'Authorization'),
...
function validateReportHistoryBody(body) {
  const issues = [];
  ...
  if (!isObject(body.run)) issues.push('run is required.');
  if (body.grouped_snapshots !== undefined && !Array.isArray(body.grouped_snapshots)) {
    issues.push('grouped_snapshots must be an array when provided.');
  }
  if (body.subjective_reviews !== undefined && !Array.isArray(body.subjective_reviews)) {
    issues.push('subjective_reviews must be an array when provided.');
  }
  if (body.consolidated_report !== undefined && !isObject(body.consolidated_report)) {
    issues.push('consolidated_report must be an object when provided.');
  }
  if (body.delivery_audit !== undefined && !isObject(body.delivery_audit)) {
    issues.push('delivery_audit must be an object when provided.');
  }

  if (!safeString(body.run?.run_id)) issues.push('run.run_id is required.');
  if (!safeString(body.run?.report_key)) issues.push('run.report_key is required.');
  if (!safeString(body.run?.report_date)) issues.push('run.report_date is required.');
  if (!safeString(body.run?.account_id)) issues.push('run.account_id is required.');
  if (!safeString(body.run?.requested_at)) issues.push('run.requested_at is required.');

  const persistableCount =
    asArray(body.grouped_snapshots).length +
    asArray(body.subjective_reviews).length +
    (isObject(body.consolidated_report) ? 1 : 0) +
    (isObject(body.delivery_audit) ? 1 : 0);

  if (persistableCount === 0) {
    issues.push('At least one of grouped_snapshots, subjective_reviews, consolidated_report or delivery_audit must be provided.');
  }
...
function buildReportGroupStatement(env, snapshot) {
  return env.META_ADS_DB.prepare(`
    INSERT INTO report_group_snapshots (
      snapshot_key, report_key, report_date, account_id, account_name, group_id,
      entity_type, category, route, selection_count, candidate_count, has_subjective,
      subjective_status, math_block_json, entities_json, pipeline_audit_json,
      account_overview_json, delivery_target_json, source_json, created_at, updated_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(snapshot_key) DO UPDATE SET
      ...
      has_subjective = excluded.has_subjective,
      subjective_status = excluded.subjective_status,
      math_block_json = excluded.math_block_json,
      entities_json = excluded.entities_json,
      pipeline_audit_json = excluded.pipeline_audit_json,
      account_overview_json = excluded.account_overview_json,
      delivery_target_json = excluded.delivery_target_json,
      source_json = excluded.source_json,
      updated_at = excluded.updated_at
  `).bind(
    safeString(snapshot.snapshot_key),
    safeString(snapshot.report_key),
    safeString(snapshot.report_date),
    safeString(snapshot.account_id),
    nullableString(snapshot.account_name),
    safeString(snapshot.group_id),
    safeString(snapshot.entity_type),
    safeString(snapshot.category),
    nullableString(snapshot.route),
    safeInteger(snapshot.selection_count),
    safeInteger(snapshot.candidate_count),
    snapshot.has_subjective ? 1 : 0,
    safeString(snapshot.subjective_status),
    safeJsonString(snapshot.math_block_json, '{}'),
    safeJsonString(snapshot.entities_json, '[]'),
    safeJsonString(snapshot.pipeline_audit_json, '{}'),
    safeJsonString(snapshot.account_overview_json, '{}'),
    safeJsonString(snapshot.delivery_target_json, '{}'),
    safeJsonString(snapshot.source_json, '{}'),
    safeString(snapshot.created_at || new Date().toISOString()),
    safeString(snapshot.updated_at || new Date().toISOString())
  );
}

function buildReportSubjectiveReviewStatement(env, review) {
  return env.META_ADS_DB.prepare(`
    INSERT INTO report_subjective_reviews (
      review_key, report_key, report_date, account_id, account_name, group_id,
      entity_type, category, subjective_status, visual_evidence_status,
      overall_subjective_verdict, recommended_creative_direction, subjective_summary,
      top_risks_json, top_opportunities_json, subjective_scores_json, review_json,
    ...
    ON CONFLICT(review_key) DO UPDATE SET
      ...
      overall_subjective_verdict = excluded.overall_subjective_verdict,
      recommended_creative_direction = excluded.recommended_creative_direction,
      ...
      subjective_scores_json = excluded.subjective_scores_json,
      review_json = excluded.review_json,
      updated_at = excluded.updated_at
  `).bind(
    safeString(review.review_key),
    safeString(review.report_key),
    safeString(review.report_date),
    safeString(review.account_id),
    nullableString(review.account_name),
    safeString(review.group_id),
    safeString(review.entity_type),
    safeString(review.category),
    safeString(review.subjective_status),
    safeString(review.visual_evidence_status),
    safeString(review.overall_subjective_verdict),
    safeString(review.recommended_creative_direction),
    safeString(review.subjective_summary),
    safeJsonString(review.top_risks_json, '[]'),
    safeJsonString(review.top_opportunities_json, '[]'),
    safeJsonString(review.subjective_scores_json, '{}'),
    safeJsonString(review.review_json, '{}'),
    safeString(review.created_at || new Date().toISOString()),
    safeString(review.updated_at || new Date().toISOString())
  );
}

function buildConsolidatedReportStatement(env, report) {
  return env.META_ADS_DB.prepare(`
    INSERT INTO consolidated_reports (
      report_key, report_date, account_id, account_name, message_type, route,
      idempotency_key, headline_math_summary, sections_json, whatsapp_text,
      group_counts_by_category_json, subjective_coverage_json, delivery_target_json,
      consolidated_groups_count, source_json, created_at, updated_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(report_key) DO UPDATE SET
      ...
      headline_math_summary = excluded.headline_math_summary,
      sections_json = excluded.sections_json,
      whatsapp_text = excluded.whatsapp_text,
      group_counts_by_category_json = excluded.group_counts_by_category_json,
      subjective_coverage_json = excluded.subjective_coverage_json,
      delivery_target_json = excluded.delivery_target_json,
      consolidated_groups_count = excluded.consolidated_groups_count,
      source_json = excluded.source_json,
      updated_at = excluded.updated_at
  `).bind(
    safeString(report.report_key),
    safeString(report.report_date),
    safeString(report.account_id),
    nullableString(report.account_name),
    safeString(report.message_type),
    safeString(report.route),
    safeString(report.idempotency_key),
    safeString(report.headline_math_summary),
    safeJsonString(report.sections_json, '[]'),
    safeString(report.whatsapp_text),
    safeJsonString(report.group_counts_by_category_json, '{}'),
    safeJsonString(report.subjective_coverage_json, '{}'),
    safeJsonString(report.delivery_target_json, '{}'),
    safeInteger(report.consolidated_groups_count),
    safeJsonString(report.source_json, '{}'),
    safeString(report.created_at || new Date().toISOString()),
    safeString(report.updated_at || new Date().toISOString())
  );
}

function buildReportDeliveryAuditStatement(env, audit) {
  return env.META_ADS_DB.prepare(`
    INSERT INTO report_delivery_audits (
      delivery_key, report_key, report_date, account_id, message_type, idempotency_key,
      sent_at, whatsapp_text_length, group_counts_by_category_json, subjective_coverage_json,
      send_status, delivery_status, provider_message_id, provider_remote_jid,
      send_response_json, created_at, updated_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(delivery_key) DO UPDATE SET
      ...
      whatsapp_text_length = excluded.whatsapp_text_length,
      group_counts_by_category_json = excluded.group_counts_by_category_json,
      subjective_coverage_json = excluded.subjective_coverage_json,
      send_status = excluded.send_status,
      delivery_status = excluded.delivery_status,
      provider_message_id = excluded.provider_message_id,
      provider_remote_jid = excluded.provider_remote_jid,
      send_response_json = excluded.send_response_json,
      updated_at = excluded.updated_at
  `).bind(
    safeString(audit.delivery_key),
    safeString(audit.report_key),
    safeString(audit.report_date),
    safeString(audit.account_id),
    safeString(audit.message_type),
    safeString(audit.idempotency_key),
    safeString(audit.sent_at || new Date().toISOString()),
    safeInteger(audit.whatsapp_text_length),
    safeJsonString(audit.group_counts_by_category_json, '{}'),
    safeJsonString(audit.subjective_coverage_json, '{}'),
    safeString(audit.send_status),
    safeString(audit.delivery_status),
    nullableString(audit.provider_message_id),
    nullableString(audit.provider_remote_jid),
    safeJsonString(audit.send_response_json, '{}'),
    safeString(audit.created_at || new Date().toISOString()),
    safeString(audit.updated_at || new Date().toISOString())
  );
}

async function upsertReportIngestionRun(env, row) {
  await env.META_ADS_DB.prepare(`
    INSERT INTO report_ingestion_runs (
      run_id, report_key, ingestion_kind, report_date, account_id, account_name,
      idempotency_key, status, grouped_snapshots_upserted, subjective_reviews_upserted,
      consolidated_reports_upserted, delivery_audits_upserted, last_error,
      summary_json, phase, last_successful_phase, attempt_count, last_request_id,
      created_at, updated_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(idempotency_key) DO UPDATE SET
      run_id = excluded.run_id,
      ...
      account_name = excluded.account_name,
      status = excluded.status,
      grouped_snapshots_upserted = excluded.grouped_snapshots_upserted,
      subjective_reviews_upserted = excluded.subjective_reviews_upserted,
      consolidated_reports_upserted = excluded.consolidated_reports_upserted,
      delivery_audits_upserted = excluded.delivery_audits_upserted,
      last_error = excluded.last_error,
      summary_json = excluded.summary_json,
      phase = excluded.phase,
      last_successful_phase = excluded.last_successful_phase,
  ...
  `).bind(
    safeString(row.run_id),
    safeString(row.report_key),
    safeString(row.ingestion_kind),
    nullableString(row.report_date),
    nullableString(row.account_id),
    nullableString(row.account_name),
    safeString(row.idempotency_key),
    safeString(row.status),
    safeInteger(row.grouped_snapshots_upserted),
    safeInteger(row.subjective_reviews_upserted),
    safeInteger(row.consolidated_reports_upserted),
    safeInteger(row.delivery_audits_upserted),
    safeString(row.last_error),
    safeJsonString(row.summary_json, '{}'),
    safeString(row.phase || 'received'),
    safeString(row.last_successful_phase),
