...
function assertCheckStatusHasPublish(results) {
  const checks = results
    .map(r => (r && r.json) ? r.json : null)
    .filter(j => j && j.phase === "checkStatus");

  for (const chk of checks) {
    const hasPublish = results.some(r => {
    ...
    if (!hasPublish) {
      throw new Error(
        `Compose (2): publish não foi gerado para platform=${chk.platform}, groupKey=${chk.groupKey}, unit=${chk.unit}`
      );
    }
  }
}
        ...
        if (isFb && firstIsVideo) {
          ...
          results.push({
            json: removeNulls({
              ...
              checkFields: "status",
              // O status sempre lê o objeto de vídeo criado em reels_start;
              // o finish pode responder sem um id reutilizável.
              statusFromPublishRunIndex: uploadRunIndexes[0] ?? null,
...
assertCheckStatusHasPublish(results);
