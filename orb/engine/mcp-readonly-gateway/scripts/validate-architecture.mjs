...
assert.match(unit, /WorkingDirectory=\/opt\/skincos\/current\/mcp-readonly-source/);
assert.match(unit, /ExecStart=\/usr\/bin\/node \/opt\/skincos\/current\/mcp-readonly-source\/server\.mjs/);
assert.doesNotMatch(unit, /current\/source\/orb\/engine\/mcp-readonly-gateway/);
