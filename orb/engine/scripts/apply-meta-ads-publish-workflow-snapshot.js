...
// Operator-only, version-checked n8n save. It refuses active workflows and
// always requires a newly exported candidate plus the observed live version.
...
if (!sourcePath || !expectedVersion) {
  throw new Error('Usage: node apply-meta-ads-publish-workflow-snapshot.js <workflow.json> --expected-version=<uuid> [--apply]');
}
...
async function main() {
  ...
  try {
    ...
    const report = {
      ...
      old_version: row.versionId,
      next_version: nextVersion,
      old_version_counter: Number(row.versionCounter),
      candidate_nodes: candidate.nodes.length,
      live_nodes: live.nodes.length,
      live_hash: stableHash({ nodes: live.nodes, connections: live.connections }),
      candidate_hash: stableHash({ nodes: candidate.nodes, connections: candidate.connections }),
      apply,
    };
    if (!apply) {
    ...
    await client.query(
      `UPDATE n8n_runtime.workflow_entity
        ...
        WHERE id=$8`,
      [JSON.stringify(candidate.nodes), JSON.stringify(candidate.connections), JSON.stringify(candidate.settings || live.settings), JSON.stringify(candidate.meta || live.meta), nextVersion, Number(row.versionCounter) + 1, now, WORKFLOW_ID],
    );
    await client.query(
      ...
      [nextVersion, WORKFLOW_ID, 'Codex CRM catalog contract', now, JSON.stringify(candidate.nodes), JSON.stringify(candidate.connections), candidate.name, candidate.description || ''],
    );
