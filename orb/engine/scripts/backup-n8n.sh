...
for command in pg_dump pg_restore psql rsync flock sha256sum; do
...
cleanup() {
  local status=$?
  rm -f "$BACKUP_MARKER"
  if [[ "$n8n_was_active" == "1" ]] && ! systemctl is-active --quiet "$SKINCOS_N8N_SERVICE"; then
    systemctl start "$SKINCOS_N8N_SERVICE" || true
  fi
  if [[ "$status" != "0" ]]; then
    rm -rf "$partial"
  fi
...
if [[ "$MANAGE_N8N_SERVICE" == "1" ]] && systemctl is-active --quiet "$SKINCOS_N8N_SERVICE"; then
  n8n_was_active=1
  systemctl stop "$SKINCOS_N8N_SERVICE"
fi
