...
const BACKUP_PATH = path.join(ROOT_DIR, 'workflows', 'meta-ads.codex-working.touoDCdMBuIhytql.before-summary-and-delivery-audit-fix.json');
...
function applyChanges(workflow) {
  ...
  getNode(workflow, 'Persist Consolidated Delivery Audit').parameters.jsCode = fs.readFileSync(
    path.join(SOURCE_DIR, 'persist-consolidated-delivery-audit.js'),
    'utf8',
  );

  getNode(workflow, 'Build Delivery History Payload').parameters.jsCode = fs.readFileSync(
    path.join(SOURCE_DIR, 'build-delivery-history-payload.js'),
    'utf8',
  );
