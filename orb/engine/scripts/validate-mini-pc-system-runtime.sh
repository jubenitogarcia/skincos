...
sudo -n -u skincos test -w "$N8N_STORAGE_PATH" && echo "skincos_can_write_storage"
...
check_no_legacy_patterns "installed orb system units" \
  ...
  "/etc/systemd/system/$SKINCOS_BACKUP_SERVICE" \
  "/etc/systemd/system/$SKINCOS_BACKUP_TIMER"
...
echo "== cloudinary execution output boundary =="
N8N_RUNTIME_HOME="$N8N_RUNTIME_HOME" node "$ROOT_DIR/scripts/patch-cloudinary-node-output.js"

echo "== execution persistence audit =="
N8N_STORAGE_PATH="$N8N_STORAGE_PATH" node "$ROOT_DIR/scripts/audit-execution-persistence.js"
...
# A cold WSL/DrvFS start rebuilds the workflow dependency index and can take
# slightly over two minutes even when the process is making healthy progress.
check_url "n8n local" "$N8N_HEALTH_URL" 36 5
check_status_code "n8n auth surface" "$N8N_AUTH_SURFACE_URL" "200 401" 36 5
