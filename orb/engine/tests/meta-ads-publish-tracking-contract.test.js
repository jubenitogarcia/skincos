...
test('creative readback accepts exact URL tags and blocks a mismatched website tag before stage', () => {
  ...
  assert.equal(verified[0].json.creative_tracking_verification.status, 'verified');
  assert.equal(verified[0].json.creative_tracking_verification.graph_request_method, 'GET');

  assert.throws(
    () => executeSource('attach-advantage-plus-verification.js', {
      input: [{ json: { ...response, operation: { ...response.operation, result: { ...response.operation.result, url_tags: 'utm_source=meta&utm_medium=paid_social&utm_campaign=wrong' } } }, pairedItem: { item: 0 } }],
      items: { 'Attach Creative Result': [{ json: sourceRow }], 'Validate Meta Placement Eligibility': [] },
    }),
...
test('WhatsApp creatives remain URL-tag not-applicable and do not turn into website conversions', () => {
  ...
  const output = executeSource('attach-advantage-plus-verification.js', {
    input: [{ json: { ok: true, operation: { status: 'completed', result: { id: sourceRow.creative_id, degrees_of_freedom_spec: { creative_features_spec: {} } } } }, pairedItem: { item: 0 } }],
    items: { 'Attach Creative Result': [{ json: sourceRow }], 'Validate Meta Placement Eligibility': [] },
  });
  assert.equal(output[0].json.creative_tracking_verification.status, 'not_applicable');
  ...
  assert.equal(inherited[0].json.creative_tracking_verification.status, 'not_applicable');
  assert.equal(inherited[0].json.creative_tracking_verification.inherited_url_tags_observed, true);
});
    ...
    creativePayload: {
      ...
      object_story_spec: {
        ...
        link_data: {
          ...
          call_to_action: { type: 'BOOK_NOW', value: { link: 'https://espacofacial.com/agendamento?unit=novo-hamburgo' } },
          multi_share_end_card: false,
          multi_share_optimized: false,
          child_attachments: [
            { image_hash: 'hash-one', name: 'Primeiro passo', description: 'Descricao um', link: 'https://espacofacial.com/agendamento?unit=novo-hamburgo' },
            { image_hash: 'hash-two', name: 'Segundo passo', description: 'Descricao dois', link: 'https://espacofacial.com/agendamento?unit=novo-hamburgo' },
          ],
        },
      },
    },
    adPayload: { name: 'Tracking | Novo Hamburgo', status: 'ACTIVE', adset_id: '323456789' },
    action: 'create_new',
