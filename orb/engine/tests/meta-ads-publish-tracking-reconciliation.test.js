...
} = require("../scripts/patch-meta-ads-tracking-reconciliation");
const {
  assertCodeSourceCoverage,
} = require("../scripts/lib/meta-ads-publish-code-sources");
...
function executeSource(
  name,
  { input = [], items = {}, execution = { id: "tracking-test" } } = {},
) {
...
function workflowUrlTagsValidator(name) {
  ...
  const match = text.match(
    /(const URL_TAG_PARAMETER_KEY_PATTERN[\s\S]*?function validUrlTags\(value\) \{[\s\S]*?^\})\n\nfunction trackingFingerprint/m,
  );
...
function websitePendingJob() {
  return {
    ...
    adPayload: {
      name: "Tracking | BarraShoppingSul",
      status: "ACTIVE",
      adset_id: "323456789",
    },
...
test("prepare sends only the redacted reconciliation request and never a Pixel/dataset payload", () => {
  ...
  const output = executeSource("prepare-tracking-reconciliation.js", {
    input: [{ json: job }],
  });
  ...
  assert.deepEqual(request, {
    ...
    operation_key:
      "tracking-adset:v1:run-tracking-test:123456789:323456789:website_schedule_v1",
    token_id: "facebook_tracking",
    account_id: "123456789",
    api_version: "v25.0",
    object_id: "323456789",
  ...
  assert.equal(
    JSON.stringify(request).match(/pixel|dataset|offline_conversion/i),
    null,
  );
});
...
test("prepare replays an already-attested Website reconciliation during resume", () => {
  ...
  const prepared = executeSource("prepare-tracking-reconciliation.js", {
    input: [{ json: job }],
  });
  assert.equal(
    prepared[0].json.gateway_request.action,
  ...
  assert.equal(
    attached[0].json.tracking_contract.reconciliation_status,
    "verified",
  );
  assert.throws(
    () => executeSource("build-stage-batch.js", { input: attached }),
    /Readback de tracking pre-stage ausente/,
  );

  const preStagePrepared = executeSource(
    "prepare-pre-stage-tracking-reconciliation.js",
    { input: attached },
  );
  assert.equal(
    preStagePrepared[0].json.gateway_request.operation_key,
    prepared[0].json.gateway_request.operation_key,
    "resume revalidation must reopen the same guarded Token Vault operation",
  );
  const revalidated = executeSource(
    "attach-pre-stage-tracking-reconciliation.js",
    {
      input: [
        {
          json: {
      ...
      items: {
        "Prepare Pre-Stage Tracking Reconciliation": preStagePrepared,
        "Attach Advantage+ Verification": attached,
      },
    },
  );
  assert.equal(
    revalidated[0].json.pre_stage_adset_conversion_reconciliation.status,
    "verified",
  );
  assert.doesNotThrow(() =>
    executeSource("build-stage-batch.js", { input: revalidated }),
  );
});

test("prepare accepts every checkpointed Website reconciliation state for an idempotent resume", () => {
  const states = [
    ["pending", "pending_reconciliation", "pending_reconciliation"],
    ["verified", "configured", "configured"],
    ["reconciled", "configured", "configured"],
  ];
  const operationKeys = new Set();
  for (const [reconciliationStatus, websiteStatus, offlineStatus] of states) {
    const job = websitePendingJob();
    job.tracking_contract.reconciliation_status = reconciliationStatus;
    job.tracking_contract.website_event_status = websiteStatus;
    job.tracking_contract.offline_event_dataset_status = offlineStatus;
    const checkpoint = executeSource("build-resume-jobs-checkpoint.js", {
      input: [{ json: job }],
    });
    const checkpointed = JSON.parse(
      JSON.stringify(
        checkpoint[0].json.checkpoint_request.summary.resume_jobs[0],
      ),
    );
    const prepared = executeSource("prepare-tracking-reconciliation.js", {
      input: [{ json: checkpointed }],
    });
    assert.equal(prepared.length, 1, reconciliationStatus);
    assert.equal(
      prepared[0].json.gateway_request.action,
      "ensure_adset_conversion_contract",
      reconciliationStatus,
    );
    operationKeys.add(prepared[0].json.gateway_request.operation_key);
  }
  // Status is a checkpoint concern, not part of the target identity. Replays
  // must retain the same Token Vault operation key and therefore the same
  // snapshot/Graph mutation boundary.
...
test("attach requires an attested Website event and offline dataset before checkpointing", () => {
  ...
  assert.equal(output.length, 1);
  assert.equal(
    output[0].json.tracking_contract.reconciliation_status,
    "reconciled",
  );
  assert.equal(
    output[0].json.tracking_contract.website_event_status,
    "configured",
  );
  assert.equal(
    output[0].json.tracking_contract.offline_event_dataset_status,
    "configured",
  );
  assert.equal(
    output[0].json.creativePayload.url_tags,
  ...
  assert.throws(
    () =>
      executeSource("attach-tracking-reconciliation.js", {
        input: [
          {
            json: {
              ok: true,
...
test("a Website profile without a conversion requirement remains compatible and is still reconciled", () => {
  ...
  const revalidated = executeSource(
    "attach-pre-stage-tracking-reconciliation.js",
    {
      ...
      items: {
        "Prepare Pre-Stage Tracking Reconciliation": preStagePrepared,
        "Attach Advantage+ Verification": output,
      },
    },
  );
  assert.doesNotThrow(() =>
...
test("stage rejects a pending Website reconciliation and permits only the attached attestation", () => {
  ...
  verified.tracking_contract.reconciliation_fingerprint = "fnv1a:pre-stage";
  assert.throws(
    () =>
      executeSource("build-stage-batch.js", { input: [{ json: verified }] }),
    /Readback de tracking pre-stage ausente/,
  );
  const preStageVerified = {
    ...verified,
    pre_stage_adset_conversion_reconciliation: {
      status: "verified",
      destination_kind: "website",
      profile_ref: "website_schedule_v1",
      website_event: { configured: true, required: true },
      offline_event_dataset: { configured: true, required: true },
      tracking_fingerprint: "fnv1a:pre-stage",
      graph_mutation: "none",
    },
  };
  const staged = executeSource("build-stage-batch.js", {
    input: [{ json: preStageVerified }],
  });
  assert.equal(staged[0].json.gateway_request.action, "stage_batch");
  assert.deepEqual(
    staged[0].json.gateway_request.jobs[0] && {
      workflow_contract_revision:
        staged[0].json.gateway_request.jobs[0].workflow_contract_revision,
      destination_kind: staged[0].json.gateway_request.jobs[0].destination_kind,
      destination_adset_id:
        staged[0].json.gateway_request.jobs[0].destination_adset_id,
      profile_ref: staged[0].json.gateway_request.jobs[0].profile_ref,
      url_tags: staged[0].json.gateway_request.jobs[0].url_tags,
    },
    {
      workflow_contract_revision:
        "meta_destination_contract_v20_tracking_reconciliation",
  ...
  assert.equal(
    Object.prototype.hasOwnProperty.call(
      staged[0].json.gateway_request.jobs[0].ad_payload,
      "destination_adset_id",
    ),
    false,
  );
...
test("creative operations carry the exact raw Website tag and private-contract selectors to the Token Vault", () => {
  const job = websitePendingJob();
  const prepared = executeSource("prepare-creative-operation.js", {
    input: [{ json: job }],
  });
  const request = prepared[0].json.gateway_request;
  assert.deepEqual(
    {
      workflow_contract_revision: request.workflow_contract_revision,
      destination_kind: request.destination_kind,
      destination_adset_id: request.destination_adset_id,
      profile_ref: request.profile_ref,
      url_tags: request.url_tags,
      payload_url_tags: request.payload.url_tags,
    },
    {
      workflow_contract_revision:
        "meta_destination_contract_v20_tracking_reconciliation",
      ...
      profile_ref: "website_schedule_v1",
      url_tags: "key1=value1&key2=value2%20encoded",
      payload_url_tags: "key1=value1&key2=value2%20encoded",
    },
  );
  assert.equal(
    request.url_tags.includes("%2520"),
    false,
    "the raw fragment must not be double-encoded",
  );
});

test("Click-to-WhatsApp sends an explicit exempt contract with no URL tags", () => {
  const job = websitePendingJob();
  ...
  assert.deepEqual(
    {
      workflow_contract_revision:
        prepared[0].json.gateway_request.workflow_contract_revision,
      destination_kind: prepared[0].json.gateway_request.destination_kind,
      profile_ref: prepared[0].json.gateway_request.profile_ref,
      url_tags: prepared[0].json.gateway_request.url_tags,
    },
    {
      workflow_contract_revision:
        "meta_destination_contract_v20_tracking_reconciliation",
...
test("workflow patch is idempotent and keeps the initial and fresh pre-stage reconciliations on their guarded paths", () => {
  const workflowPath = path.join(
    __dirname,
    "..",
    "workflows",
    "meta-ads-publish.current.json",
  );
  ...
  for (const nodeName of [
    ...
    assert.equal(
      twice.nodes.filter((node) => node.name === nodeName).length,
      1,
    );
  }
  const targets = (name) =>
    (twice.connections[name]?.main || []).flat().map((edge) => edge.node);
  ...
  assert.deepEqual(targets("Attach Advantage+ Verification"), [
    PRE_STAGE_PREPARE_NODE,
  ]);
  assert.deepEqual(targets(PRE_STAGE_PREPARE_NODE), [PRE_STAGE_ENSURE_NODE]);
  assert.deepEqual(targets(PRE_STAGE_ENSURE_NODE), [PRE_STAGE_ATTACH_NODE]);
  assert.deepEqual(targets(PRE_STAGE_ATTACH_NODE), ["Build Stage Batch"]);
});
