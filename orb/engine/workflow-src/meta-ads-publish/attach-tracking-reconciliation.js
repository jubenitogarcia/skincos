...
function attestedTrackingContract(sourceTracking, result, group) {
  ...
  if (kind === 'whatsapp') {
    if (status !== 'not_applicable' || text(result.destination_kind) !== 'whatsapp') {
      throw new Error(`Attach Tracking Reconciliation recebeu resultado WhatsApp invalido: ${JSON.stringify({ status, destination_kind: text(result.destination_kind) })}`);
    }
    return {
      ...tracking,
  ...
  if (!['verified', 'reconciled'].includes(status) || text(result.destination_kind) !== 'website' || text(result.profile_ref) !== text(group.profile_ref)) {
    throw new Error(`Attach Tracking Reconciliation nao confirmou o perfil Website: ${JSON.stringify({ status, destination_kind: text(result.destination_kind), profile_ref: text(result.profile_ref) })}`);
  }
  ...
  if (website.required !== websiteEventRequired || (websiteEventRequired && website.configured !== true)) {
    throw new Error('Attach Tracking Reconciliation nao confirmou o evento Website requerido.');
  }
  if (text(tracking.offline_event_dataset_requirement) === 'required' && (offline.configured !== true || offline.required !== true)) {
    throw new Error('Attach Tracking Reconciliation nao confirmou o dataset offline requerido.');
  }
  return {
    ...tracking,
    destination_kind: 'website',
    website_event_status: websiteEventRequired ? 'configured' : 'not_required',
    offline_event_dataset_status: text(tracking.offline_event_dataset_requirement) === 'required' ? 'configured' : 'not_required',
...
const prepared = $items('Prepare Tracking Reconciliation') || [];
const validated = $items('Validate Meta Creative Payload') || [];
if (!prepared.length || !validated.length) throw new Error('Attach Tracking Reconciliation nao encontrou contexto validado.');
...
for (const [index, item] of $input.all().entries()) {
  ...
  if (response.ok !== true || text(object(response.operation).status) !== 'completed') {
    throw new Error(`Ensure Ad Set Conversion Contract falhou: ${JSON.stringify(response.detail || response.error || response)}`);
  }
  const safeResult = {
  ...
  for (const sourceIndex of list(group.job_indexes)) {
    const sourceItem = validated[Number(sourceIndex)] || {};
    const source = object(sourceItem.json);
    if (!Object.keys(source).length) throw new Error(`Attach Tracking Reconciliation perdeu job validado ${sourceIndex}.`);
    const tracking = attestedTrackingContract(source.tracking_contract, safeResult, group);
    outputs.push({
      json: {
        ...source,
        tracking_contract: tracking,
        adset_conversion_reconciliation: safeResult,
      },
...
if (outputs.length !== validated.length) {
  throw new Error(`Attach Tracking Reconciliation produziu cobertura incompleta: esperado ${validated.length}, recebeu ${outputs.length}.`);
}
return outputs;
