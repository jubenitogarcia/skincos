...
const SAFE_LANDING_PAGE_BY_DESTINATION = Object.freeze({
  barrashoppingsul:
    "https://espacofacial.com/agendamento?unit=barrashoppingsul",
...
const WORKFLOW_CONTRACT_REVISION =
  "meta_destination_contract_v20_tracking_reconciliation";

const root = $input.first()?.json || {};
if (root.ok !== true || root.ready !== true) {
  throw new Error(
    `Meta Publish gateway nao esta pronto: ${JSON.stringify(root.invalid || root.error || {})}`,
  );
}

const gatewayContractRevision = safeString(
  asObject(root.capabilities).workflow_contract_revision,
);
if (gatewayContractRevision !== WORKFLOW_CONTRACT_REVISION) {
  throw new Error(
    `Meta Publish gateway com revisao de contrato divergente: esperado ${WORKFLOW_CONTRACT_REVISION}, recebido ${gatewayContractRevision || "ausente"}.`,
  );
}
...
const supportedVideoActions = Array.isArray(videoUpload.supported_actions)
  ...
  : [];
const missingVideoActions = requiredVideoActions.filter(
  (action) => !supportedVideoActions.includes(action),
);
if (missingVideoActions.length) {
  throw new Error(
    `Meta Publish gateway sem contrato de upload de video: ${missingVideoActions.join(", ")}.`,
  );
}
if (
  Number(videoUpload.max_file_bytes) < 90 * 1024 * 1024 ||
  Number(videoUpload.max_chunk_bytes) < 16 * 1024 * 1024
) {
  throw new Error(
    "Meta Publish gateway declarou limites insuficientes para upload de video.",
  );
}

const trackingCapabilities = asObject(asObject(root.capabilities).tracking);
if (
  trackingCapabilities.adset_conversion_reconciliation !== true ||
  trackingCapabilities.creative_url_tags_readback !== true
) {
  throw new Error(
    "Meta Publish gateway sem capacidade de reconciliacao de conversao e url_tags.",
  );
}
const trackingBindingRevision = safeString(
  root.tracking_binding_revision || root.config_revision,
);
if (!/^[a-f0-9]{64}$/i.test(trackingBindingRevision)) {
  throw new Error(
    "Meta Publish gateway sem revisao estavel de vinculacao de tracking.",
  );
}
...
const rows = destinations
  .map((entry) => {
    ...
    if (missing.length) {
      throw new Error(
        `Meta Publish config incompleta em ${item.token_id || item.destination_group || "destino"}: ${missing.join(", ")}`,
      );
    }
    const configuredLandingPages = asObject(
      item.landing_pages_by_creative_group,
    );
    const destinationKey = normalizeCompact(item.destination_group);
    const safeFallbackLandingPage =
      SAFE_LANDING_PAGE_BY_DESTINATION[destinationKey] || "";
    if (
      !Object.keys(configuredLandingPages).length &&
      !safeFallbackLandingPage
    ) {
      throw new Error(
        `Meta Publish sem landing page segura para o destino ${safeString(item.destination_group)}.`,
      );
    }
    ...
    return {
      json: {
        ...
        allowed_link_hosts: Array.isArray(item.allowed_link_hosts)
          ? item.allowed_link_hosts
          : [],
        landing_pages_by_creative_group: landingPages,
        tracking_contract: asObject(item.tracking_contract),
        ...
        whatsapp_destination_url: safeString(
          item.whatsapp_destination_url || SAFE_WHATSAPP_DESTINATION_URL,
        ),
        carousel_native_campaign_id: safeString(
          item.carousel_native_campaign_id,
        ),
        carousel_native_adset_id: safeString(item.carousel_native_adset_id),
        carousel_native_adset_verified:
          item.carousel_native_adset_verified === true,
        carousel_native_route_active:
          item.carousel_native_route_active === true,
        config_revision: safeString(root.config_revision),
        tracking_binding_revision: trackingBindingRevision,
  ...
  .sort(
    (left, right) =>
      Number(left.json.row_number || 0) - Number(right.json.row_number || 0),
  );

if (rows.length < 2) {
  throw new Error(
    `Meta Publish gateway precisa de 2 destinos ativos; encontrados ${rows.length}.`,
  );
}
