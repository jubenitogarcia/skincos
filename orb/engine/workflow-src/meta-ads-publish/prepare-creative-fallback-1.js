const ATTEMPT = 1;
const OPTIONAL_REMOVAL_PRIORITY = [
  "pac_relaxation",
  "music_generation",
  "video_highlights",
  "video_filtering",
...
function responseSucceeded(response) {
  return (
    ...
    text(response.operation.result && response.operation.result.id)
  );
}
function responseText(response) {
  try {
    ...
    return text(response).toLowerCase();
  }
}
function chooseRemoval(features, response) {
  const available = OPTIONAL_REMOVAL_PRIORITY.filter((feature) =>
  ...
  return /(creative[ _-]?feature|advantage\+?|enhancement|text[ _-]?overlay|music|pac[ _-]?relaxation|video[ _-]?(filtering|highlights?)|adapt[ _-]?to[ _-]?placement)/.test(
    detail,
  )
    ? available[0] || ""
    : "";
...
function markRemoved(groups, feature, reason) {
  ...
  for (const group of ["main", "essential", "supplemental"]) {
    out[group] = list(out[group]).map((entry) =>
      text(entry && entry.api_key) === feature
...
function trackingGatewayFields(source, payload) {
  ...
  if (!["website", "whatsapp"].includes(destinationKind))
    throw new Error(
      `Fallback de Creative recebeu destino de tracking invalido: ${source.job_key || "sem-chave"}.`,
    );
  return {
    workflow_contract_revision: text(source.workflow_contract_revision),
    destination_kind: destinationKind,
    destination_adset_id: text(source.destination_adset_id),
    profile_ref:
...
return $input.all().map((item, index) => {
  ...
  const resolvedCreativeId = succeeded
    ? text(response.operation.result.id)
    : "";
  if (succeeded) {
    return {
      json: {
        ...
        gateway_request: {
          action: "get_creative",
          operation_key: key(
            `fallback-read-${ATTEMPT}:${source.run_id}:${resolvedCreativeId}`,
          ),
  ...
  const features = object(
    object(payload.degrees_of_freedom_spec).creative_features_spec,
  );
  const removedFeature = chooseRemoval(features, response);
  if (!removedFeature)
    throw new Error(
      `Create AdCreative falhou por erro estrutural ou nao mapeado; fallback de Advantage+ nao aplicado em ${source.job_key || index}.`,
    );
  ...
  const removed = unique([
    ...list(source.advantage_plus_fallback_removed_features),
    removedFeature,
  ]);
  const requested = unique(
    list(source.advantage_plus_requested_features).filter(
  ...
  return {
    json: {
      ...
      advantage_plus_feature_groups: markRemoved(
        source.advantage_plus_feature_groups,
        removedFeature,
        fallbackReason,
      ),
      ...
      creative_fallback_attempts: [
        ...
        {
          attempt: ATTEMPT,
          removed_feature: removedFeature,
      ...
      gateway_request: {
        ...
        operation_key: key(
          `creative:v3:${payloadHash}:${source.run_id}:${source.destination_group}:fallback_${ATTEMPT}:${removedFeature}`,
        ),
        token_id: text(source.token_id),
        account_id: text(source.account_id),
        api_version: text(source.api_version || "v25.0"),
        ...trackingGatewayFields(source, payload),
        payload,
      },
    },
    binary: item.binary,
