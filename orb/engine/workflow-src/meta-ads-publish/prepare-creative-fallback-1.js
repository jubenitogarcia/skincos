const ATTEMPT = 1;
const OPTIONAL_REMOVAL_PRIORITY = ['pac_relaxation', 'music_generation', 'add_text_overlay'];
...
return $input.all().map((item, index) => {
  ...
  if (!removedFeature) {
    throw new Error(`Create AdCreative falhou sem enhancement opcional removivel em ${source.job_key || index}: ${JSON.stringify(response)}`);
  }
