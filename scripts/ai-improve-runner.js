...
function loadModelsFromFile() {
    try {
        const cfgPath = join('.github', 'ai-models.json');
...
function runSmoke() {
    ...
    if (existsSync('scripts/e2e.sh')) {
        ok = tryRun('bash scripts/e2e.sh ci-smoke');
        tryRun('bash scripts/e2e.sh health');
    } else if (existsSync('scripts/ci-smoke-assert.sh')) {
        ok = tryRun('bash scripts/ci-smoke-assert.sh');
        if (existsSync('scripts/ci-repo-health.sh')) {
            tryRun('bash scripts/ci-repo-health.sh');
        }
    } else {
