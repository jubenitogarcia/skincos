...
async function collectTreeEntries(rootDir, { include = () => true } = {}) {
...
export async function fingerprintBuildInputs(consoleDir = DEFAULT_CONSOLE_DIR, { includeEntries = false } = {}) {
...
export async function fingerprintDist(consoleDir = DEFAULT_CONSOLE_DIR, { includeEntries = false } = {}) {
  ...
  const entries = await collectTreeEntries(distDir)
  const result = {
