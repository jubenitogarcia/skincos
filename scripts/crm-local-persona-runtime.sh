...
crm_persona_runtime_acquire_lock() {
  ...
  if [[ "$owner_pid" =~ ^[0-9]+$ ]] && kill -0 "$owner_pid" 2>/dev/null; then
    echo "[crm-local] Outra inicialização de $CRM_PERSONA está em andamento (PID $owner_pid)." >&2
    return 1
  fi
...
crm_persona_runtime_write_build_state() {
  ...
  CRM_BUILD_COMMIT="$commit" CRM_BUILD_UPDATED_AT="$(date -Iseconds)" node - "$CRM_BUILD_STATE_FILE" <<'NODE'
const fs = require('fs')
