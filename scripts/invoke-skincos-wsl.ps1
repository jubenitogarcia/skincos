...
try {
    ...
    $identityProbe = Invoke-WslCapture -Wsl $wsl -Arguments @("-d", $Distro, "-u", $LinuxUser, "--", "/usr/bin/id", "-un")
    ...
    if (-not $SkipRepoCheck) {
        $repoProbe = Invoke-WslCapture -Wsl $wsl -Arguments @("-d", $Distro, "-u", $LinuxUser, "--", "/usr/bin/test", "-d", $repoMountPath)
        if ($repoProbe.ExitCode -ne 0) {
            throw "Skincos project root is unavailable in WSL at '$repoMountPath'. No service was started."
        }
        $workingDirectoryProbe = Invoke-WslCapture -Wsl $wsl -Arguments @("-d", $Distro, "-u", $LinuxUser, "--", "/usr/bin/test", "-d", $executionDirectory)
    ...
    foreach ($tool in $toolChecks) {
        $toolProbe = Invoke-WslCapture -Wsl $wsl -Arguments @("-d", $Distro, "-u", $LinuxUser, "--", "/usr/bin/which", $tool)
    ...
    Write-Host "WSL backend: $Distro ($LinuxUser)"
    Write-Host "Running in WSL directory: $executionDirectory"

    $wslArguments = @("-d", $Distro, "-u", $LinuxUser, "--cd", $executionDirectory, "--") + $linuxCommand
    $previousErrorPreference = $ErrorActionPreference
    $ErrorActionPreference = "Continue"
    try {
