...
$lines = @(
    ...
    "3. O hook ``UserPromptSubmit`` entrega a primeira mensagem ao resolver como ``edit``. Prévia e qualificação usam ações explícitas; não deduza essas intenções pelas palavras da tarefa.",
    "4. Se o resultado for ``ready``, continue somente na worktree atual. Se for ``replace`` com ``currentThreadAction=create_replacement_thread``, não escreva no checkout atual: confirme o projeto pelo caminho exato, crie uma thread substituta no ambiente nativo ``worktree``, espere ``ready``, navegue para ela e só então arquive a original. ``handoff_thread`` só vale para outra thread já identificada; nunca para a própria chamadora.",
    ...
    "6. A substituta criada pelo App deve iniciar com o marcador privado emitido pelo hook; ele só é aceito no worktree gerenciado, detached e no SHA esperado. Depois de ``ready``, valide a identidade com ``powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\validate-skincos-worktree.ps1 -ProjectRoot (Get-Location).Path -TaskSlug '$taskSlugLiteral' -Mode edit``; trabalhe apenas nela e trate o clone compartilhado como somente leitura para contexto.",
