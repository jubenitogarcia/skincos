...
CRM_ROUTE="${CRM_ROUTE:-/?module=atendimento-clinica}"
...
PID_FILE="${CRM_PID_FILE:-$ROOT_DIR/.crm-atendimento-clinica-local.pid}"
LOG_FILE="${CRM_LOG_FILE:-$ROOT_DIR/.crm-atendimento-clinica-local.log}"

usage() {
  cat <<EOF
SKINCOS • Testar CRM Atend. Clínica local
...
Exemplos:
  ./scripts/run-local-atendimento-clinica.sh
  ./scripts/run-local-atendimento-clinica.sh --build
  ./scripts/run-local-atendimento-clinica.sh --smoke --exit-after-smoke
EOF
}
...
load_local_env() {
  ...
  load_env_file "$ROOT_DIR/backend/apps/crm-api/.env"
...
assert_port_free() {
  ...
  if [[ -n "$pids" ]]; then
    echo "[atendimento-clinica-local] Porta $port ocupada por $label (pid: $pids)." >&2
    echo "[atendimento-clinica-local] Rode: ./scripts/run-local-atendimento-clinica.sh --stop" >&2
    exit 1
  fi
}
...
pick_available_port() {
  ...
  while (( port < preferred + 50 )); do
    ...
    if is_port_free "$port"; then
      echo "[atendimento-clinica-local] Porta $preferred ocupada por $label; usando $port." >&2
      printf '%s' "$port"
      return 0
    fi
  done
  echo "[atendimento-clinica-local] Nenhuma porta livre encontrada para $label a partir de $preferred." >&2
  exit 1
}
...
stop_existing() {
  ...
  if [[ -n "$pid" ]] && kill -0 "$pid" >/dev/null 2>&1; then
    echo "[atendimento-clinica-local] Encerrando instancia anterior (pid: $pid)"
    terminate_tree "$pid"
    sleep 1
  fi
  rm -f "$PID_FILE"
...
stop_owned_port_listener() {
  ...
  for pid in $pids; do
    ...
    if [[ "$args" == *"$ROOT_DIR"* || "$args" == *"backend/apps/crm-api/server.js"* ]]; then
      if [[ "$args" == *"vite"* || "$args" == *"backend/apps/crm-api/server.js"* || "$args" == *"npm run dev"* ]]; then
        echo "[atendimento-clinica-local] Encerrando $label preso na porta $port (pid: $pid)"
        terminate_tree "$pid"
      fi
    fi
  done
...
if [[ "$STOP_ONLY" == "1" ]]; then
  ...
  echo "CRM Atend. Clinica local finalizado."
  exit 0
fi
...
echo ""
echo "SKINCOS • Testar CRM Atend. Clinica local"
echo "Modulo inicial: atendimento-clinica"
echo "URLs:"
echo "  Frontend : $DEFAULT_URL"
echo "  CRM API  : http://127.0.0.1:${CRM_API_PORT}/api/atendimento-clinica/health"
echo "Log: $LOG_FILE"
echo ""

if [[ -z "${DATABASE_URL:-}" ]]; then
  echo "[atendimento-clinica-local] DATABASE_URL nao configurado." >&2
  echo "[atendimento-clinica-local] Defina DATABASE_URL no shell ou em backend/config/workspace.local.env antes de abrir o modulo." >&2
  exit 1
fi

if [[ "$CRM_BUILD_BEFORE_START" == "1" ]]; then
  echo "[atendimento-clinica-local] Gerando build do frontend..."
...
(
  ...
  export DEV_AUTH_ALLOWED_MODULES="${DEV_AUTH_ALLOWED_MODULES:-atendimento-clinica,faturamento,procedimentos,ponto,status,users}"
...
if ! wait_for_http "http://127.0.0.1:${CRM_API_PORT}/api/auth/me" 120; then
  echo "[atendimento-clinica-local] crm-api nao respondeu em tempo habil. Veja $LOG_FILE" >&2
  exit 1
fi
...
(
  ...
  export VITE_LOCAL_CRM_FOCUS_MODULE=atendimento-clinica
...
if [[ "$CRM_OPEN_BROWSER" == "1" ]]; then
  (
    if wait_for_http "$DEFAULT_URL" 90; then
      ...
      echo "[atendimento-clinica-local] O CRM nao respondeu em $DEFAULT_URL dentro do tempo esperado."
    fi
  ) &
fi
...
if [[ "$CRM_SMOKE" == "1" ]]; then
  if ! wait_for_http "$DEFAULT_URL" 90; then
    echo "[atendimento-clinica-local] O CRM nao respondeu para a smoke em tempo habil." >&2
    exit 1
  fi
  echo "[atendimento-clinica-local] Rodando smoke local do Atend. Clinica..."
  (
    cd "$FRONTEND_DIR"
    CRM_URL="$DEFAULT_URL" HEADED="$CRM_SMOKE_HEADED" npm run smoke:atendimento-clinica:local
  )
  if [[ "$CRM_EXIT_AFTER_SMOKE" == "1" ]]; then
    echo "[atendimento-clinica-local] Smoke concluida; encerrando CRM local."
    exit 0
  fi
fi

echo "Notas:"
echo "  - O backend local roda com NO_AUTH=true e modulo atendimento-clinica liberado."
