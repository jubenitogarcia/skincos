...
if [[ -n "${CRM_LOCAL_NATIVE_SOURCE_ROOT:-}" && "${CRM_LOCAL_NATIVE_SOURCE_ROOT}" != "$ROOT_DIR" ]]; then
  case "$CRM_LOCAL_NATIVE_SOURCE_ROOT" in
    /home/admin/.local/state/skincos/crm-local-preview-source/*) ;;
    *)
      echo "CRM_LOCAL_NATIVE_SOURCE_ROOT is outside the private preview root: $CRM_LOCAL_NATIVE_SOURCE_ROOT" >&2
      exit 2
      ;;
  esac

  mkdir -p "$(dirname "$CRM_LOCAL_NATIVE_SOURCE_ROOT")"
  rsync -a --delete \
    --exclude '.git' \
    --exclude 'node_modules' \
    --exclude 'test-results' \
    "$ROOT_DIR/" "$CRM_LOCAL_NATIVE_SOURCE_ROOT/"

  export CRM_LOCAL_NATIVE_SOURCE_ROOT=''
  exec bash "$CRM_LOCAL_NATIVE_SOURCE_ROOT/scripts/run-local-atendimento.sh" "$@"
fi
