...
ensure_playwright_chromium() {
  ...
  (
    cd "$FRONTEND_DIR"
    PLAYWRIGHT_BROWSERS_PATH="$CRM_PLAYWRIGHT_BROWSERS_PATH" npm exec playwright install chromium
  )
}
...
if [[ "$CRM_SMOKE" == "1" ]]; then
  if [[ "$CRM_MODULE" == "meta-ads" ]]; then
    ...
    run_browser_smoke env PLAYWRIGHT_BROWSERS_PATH="$CRM_PLAYWRIGHT_BROWSERS_PATH" CRM_URL="$DEFAULT_URL" META_ADS_LOCAL_SCENARIO="${CRM_META_ADS_SCENARIO:-connected-ready}" HEADED="$CRM_SMOKE_HEADED" npm run smoke:meta-ads:local
  elif [[ "$CRM_MODULE" == "site-tracking" ]]; then
    echo "[crm-local] Rodando smoke local do Site EF..."
    run_browser_smoke env PLAYWRIGHT_BROWSERS_PATH="$CRM_PLAYWRIGHT_BROWSERS_PATH" CRM_URL="$DEFAULT_URL" META_ADS_LOCAL_SCENARIO="${CRM_META_ADS_SCENARIO:-connected-ready}" HEADED="$CRM_SMOKE_HEADED" npm run smoke:site-tracking:local
  else
    echo "[crm-local] Rodando smoke local padrão..."
    run_browser_smoke env PLAYWRIGHT_BROWSERS_PATH="$CRM_PLAYWRIGHT_BROWSERS_PATH" CRM_URL="$DEFAULT_URL" HEADED="$CRM_SMOKE_HEADED" node ./scripts/crm-local-smoke.cjs
  fi
