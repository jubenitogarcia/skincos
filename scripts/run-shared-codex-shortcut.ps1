...
function Invoke-ShortcutWslNativePreview {
    ...
    if ($LASTEXITCODE -ne 0) { throw "The native WSL preview command failed with exit code $LASTEXITCODE." }
}
...
function Start-ShortcutWslDetached {
    ...
    $arguments = @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $wslInvoker, '-RepoCommand', $Command, '-ProjectRoot', $WorkingProjectRoot)
    if ($SkipBootstrapCheck) { $arguments += '-SkipBootstrapCheck' }
...
function Start-CrmPersonaRuntime {
    ...
    [void](Start-ShortcutWslDetached -WorkingProjectRoot $SourceRoot -SkipBootstrapCheck -Command $command -OutputLog $personaLauncherOut -ErrorLog $personaLauncherErr)
}
...
function Start-CrmAtendimentoRuntime {
    ...
    $command = "CRM_PERSONA=GESTOR CRM_TARGET_COMMIT={0} CRM_SOURCE_FINGERPRINT={1} CRM_RUNTIME_ROOT={2} CRM_LOCAL_NATIVE_SOURCE_ROOT={3} LOCAL_AUTH_BYPASS=true LOCAL_AUTH_TEST_USER_ADMIN=true LOCAL_AUTH_ROLE=GESTOR LOCAL_AUTH_EMAIL=dev@local.test LOCAL_AUTH_NAME='Gestor Local' CRM_WITH_INSUMOS=0 CRM_WITH_TIMEKEEPING=0 CRM_WITH_WHATSAPP=1 CRM_BUILD_BEFORE_START=1 CRM_OPEN_BROWSER=1 CRM_PID_FILE={4} CRM_LOG_FILE={5} bash ./scripts/run-local-atendimento.sh" -f `
    ...
    [void](Start-ShortcutWslDetached -WorkingProjectRoot $SourceRoot -SkipBootstrapCheck -Command $command -OutputLog $atendimentoLauncherOut -ErrorLog $atendimentoLauncherErr)
}
