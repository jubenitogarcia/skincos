...
function Invoke-ShortcutActionInternal {
    ...
    switch ($SelectedAction) {
        ...
        "CrmAtendimento" {
            $crmAtendimentoLaunch = Resolve-CrmLocalModuleSourceRoot -Persona Gestor
            $snapshotId = ($crmAtendimentoLaunch.SourceFingerprint -split ':')[-1]
            if ([string]::IsNullOrWhiteSpace($snapshotId) -or $snapshotId.Length -lt 12) {
                throw "Fingerprint inválido para a prévia isolada de Atendimento: '$($crmAtendimentoLaunch.SourceFingerprint)'."
            }
            $nativeAtendimentoSource = "/home/admin/.local/state/skincos/crm-local-preview-source/atendimento-{0}-{1}" -f `
                $crmAtendimentoLaunch.TargetCommit.Substring(0, 12), $snapshotId.Substring(0, 12)
            Invoke-ShortcutWsl -WorkingProjectRoot $crmAtendimentoLaunch.SourceRoot -Command ("CRM_TARGET_COMMIT={0} CRM_SOURCE_FINGERPRINT={1} CRM_LOCAL_NATIVE_SOURCE_ROOT={2} CRM_PID_FILE={3} CRM_LOG_FILE={4} bash ./scripts/run-local-atendimento.sh" -f `
                (Convert-ToBashLiteral -Value $crmAtendimentoLaunch.TargetCommit), `
                (Convert-ToBashLiteral -Value $crmAtendimentoLaunch.SourceFingerprint), `
