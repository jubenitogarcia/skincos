...
# Performs the only supported manual restart of Orb.
# Livia observes the lock before creating a publish graph; existing Livia
# executions are allowed to reach a terminal state before Orb is fenced and
# restarted.
...
# Defaults can be overridden for testing and maintenance windows.
...
Creates a fail-closed Livia maintenance window, waits for current Livia executions to
finish, then restarts only orb.service. It never forces a restart through an active publication.
EOF
}
...
[[ "$timeout_seconds" =~ ^[1-9][0-9]{0,3}$ ]] && (( timeout_seconds <= 3600 )) || {
 ...
 [[ "$poll_seconds" =~ ^[1-9][0-9]?$ ]] && (( poll_seconds <= 60 )) || {
   echo '--poll-seconds must be an integer from 1 through 60.' >&2
   exit 2
 }
...
sudo -n -u skincos bash -c 'umask 027; printf "reason=%s\nstarted_at=%s\n" "$1" "$2" >"$3"' \
  controlled_orb_restart "$(date --iso-8601=seconds)" "$LOCK_FILE"

active_livia_count() {
  sudo -n -u postgres psql -d n8n_runtime -Atqc "
    SELECT count(*)
    FROM n8n_runtime.execution_entity
...
while true; do
  ...
  [[ "$active" =~ ^[0-9]+$ ]] || {
    echo 'Unable to determine active Livia executions.' >&2
    exit 1
  }
  if (( active == 0 )); then
    break
...
# RefuseManualStop=yes on orb.service rejects accidental direct stops.
# The fence creates an indirect, ordered stop only after the drain above; clearing
