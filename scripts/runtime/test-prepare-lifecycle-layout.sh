...
bash -n "$SCRIPT"
output="$(STATE_ROOT=/tmp/skincos-state CONFIG_ROOT=/tmp/skincos-config LOG_ROOT=/tmp/skincos-log TMP_ROOT=/tmp/skincos-tmp BACKUP_ROOT=/tmp/skincos-backup "$SCRIPT")"
...
if "$SCRIPT" --final-sync >/dev/null 2>&1; then
