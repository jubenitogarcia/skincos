...
test('production database contract has a separate read-only app role without raw contact access', () => {
  ...
  const productionMigration = read('scripts/run-atendimento-production-migration.sh')
  const productionMigrationRunner = read('crm/api/scripts/migrate-atendimento-production.mjs')
  const productionMigratorCli = read('crm/api/scripts/run-atendimento-production-migration.mjs')
  const productionBackup = read('scripts/backup-atendimento-production.sh')
  const releaseValidator = read('crm/api/scripts/validate-atendimento-release.mjs')
  const unit = read('ops/runtime/units/crm-atendimento-production.service')
  ...
  assert.match(provision, /MIGRATOR_CONFIG=.*crm-clientes-production-migrator\.env/)
  assert.match(provision, /alter role \$MIGRATOR_ROLE connection limit 3/)
  ...
  assert.match(provision, /BACKUP_SCRIPT="\$SCRIPT_DIR\/backup-atendimento-production\.sh"/)
  assert.match(provision, /backup_report="\$\(run_sudo_clean \/usr\/bin\/bash -p "\$BACKUP_SCRIPT"\)"/)
  assert.match(provision, /backup_created=true\\ database=skincos_clientes_production\\ sha256=\[0-9a-f\]\{64\}\\ private=true\\ unique=true/)
  assert.match(provision, /install -d -m 0700 -o root -g root "\$BACKUP_ROOT"/)
  ...
  assert.match(validation, /PRODUCTION_DEFERRAL_RELATION='crm_atendimento\.production_migration_deferrals'/)
  assert.match(validation, /PRODUCTION_SOURCE_MIRROR_NOT_PROVISIONED/)
  assert.doesNotMatch(validation, /global_client_identities/)
  assert.match(unit, /Environment=CRM_ATENDIMENTO_COMMERCIAL_SOURCE_DEFERRED=true/)
  ...
  assert.match(productionMigration, /backup-atendimento-production\.sh/)
  assert.match(productionMigration, /lockdown-atendimento-production-runtime\.sh/)
  assert.match(productionMigration, /state.*maintenance/s)
  assert.match(productionMigrationRunner, /ATENDIMENTO_MIGRATION_TARGETS\.PRODUCTION/)
  assert.match(productionMigrationRunner, /ATENDIMENTO_PRODUCTION_PREREQUISITE_DEFERRED_RULES/)
  assert.match(productionMigrationRunner, /PRODUCTION_SOURCE_MIRROR_NOT_PROVISIONED/)
  assert.match(productionMigrationRunner, /production_migration_deferrals/)
  assert.match(productionMigrationRunner, /!activeMigrationIds\.has\(migration\.id\)/)
  assert.doesNotMatch(productionMigrationRunner, /inspectAndPersistStagingDeferral/)
  assert.match(productionMigratorCli, /crm-clientes-production-migrator\.env/)
  assert.match(productionBackup, /DATABASE='skincos_clientes_production'/)
  assert.doesNotMatch(productionBackup, /--file=/)
  assert.match(releaseValidator, /!\['staging', 'production'\]\.includes\(target\)/)
})
...
test('release, rollback and tunnel paths are immutable, fixed and isolated', () => {
  ...
  assert.match(tunnel, /mktemp \/tmp\/atendimento-production-tunnel-config\.XXXXXX\.yml/)
