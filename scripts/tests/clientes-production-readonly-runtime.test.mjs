...
test('staging release, control and application role remain fixed and read-only', () => {
  ...
  assert.match(installer, /unit_backup="\$\{stamp\}-\$SERVICE"/)
  ...
  assert.match(control, /CONTROL_BACKUP_NAME="\$\{stamp\}-module-control\.json"/)
  ...
  assert.match(rollback, /--unit-backup must be a fixed staging unit backup basename/)
  assert.match(rollback, /--control-backup must be a fixed staging control backup basename/)
  ...
  assert.match(rollbackControl, /const BACKUP_NAME = \/\^\[0-9\]\{8\}T\[0-9\]\{6\}Z-module-control\\\.json\$\//)
