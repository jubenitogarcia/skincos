...
test('Atendimento shortcut uses the canonical isolated Pages and adapter runtime', () => {
  ...
  assert.match(launcher, /crmAtendimentoLaunch\.TargetCommit/)
  assert.match(launcher, /crmAtendimentoLaunch\.SourceFingerprint/)
})
...
test('all CRM module shortcuts select a current snapshot before launch', () => {
  ...
  assert.match(launcher, /"CrmAtendimento"[\s\S]*Resolve-CrmLocalModuleSourceRoot -Persona Gestor/)
})
