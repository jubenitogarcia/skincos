...
test('Codex and Windows actions expose the canonical CRM entry points', () => {
  for (const label of ['CRM – Local', 'CRM – Módulos']) {
...
test('Gestor owns every shared service on the canonical ports', () => {
  ...
  for (const variable of ['CRM_WITH_INSUMOS=1', 'CRM_WITH_TIMEKEEPING=1', 'CRM_WITH_WHATSAPP=1']) {
    assert.match(launcher, new RegExp(variable))
  }
...
test('local Pages routes Atendimento to the isolated CRM adapter', () => {
  assert.match(pagesRunner, /LOCAL_WA_ORCHESTRATOR_API_TARGET/)
  assert.match(pagesRunner, /native service on :8099/)
  assert.ok(
    pagesRunner.includes('add_binding "ATENDIMENTO_API_TARGET" "$LOCAL_WA_ORCHESTRATOR_API_TARGET"'),
    'Atendimento must use the isolated WA orchestrator binding directly',
  )
})

test('local CRM adapter uses only the peer-authenticated Atendimento mirror', () => {
  assert.match(whatsappRunner, /crm_local_wa_database_url\(\)/)
  assert.match(whatsappRunner, /POSTGRES_SOCKET_DIR="\/var\/run\/postgresql"/)
  assert.match(whatsappRunner, /export DATABASE_URL="\$expected_database_url"/)
  assert.match(whatsappRunner, /CRM_LOCAL_WA_DATABASE_URL/)
})
...
test('Atendimento uses the isolated Pages, adapter and source-fingerprint policy', () => {
  ...
  assert.doesNotMatch(launcher, /Invoke-ShortcutWslNativePreview/)
  assert.match(launcher, /-WorkingProjectRoot \$manifestWorktree/)
...
test('opening the browser never blocks the runtime manifest transition', () => {
  assert.match(crmRunner, /open_browser\(\)/)
})

test('Playwright browsers remain in the private operator runtime across npm ci', () => {
  assert.match(crmRunner, /PLAYWRIGHT_BROWSERS_PATH="\$PLAYWRIGHT_BROWSERS_PATH"/)
  assert.match(crmRunner, /PLAYWRIGHT_BROWSERS_PATH="\$PLAYWRIGHT_BROWSERS_PATH" npm exec playwright install chromium/)
  assert.doesNotMatch(crmRunner, /PLAYWRIGHT_BROWSERS_PATH=0/)
  const buildCase = crmRunner.indexOf('case "$CRM_BUILD_BEFORE_START"')
  assert.ok(buildCase >= 0)
  assert.ok(
    crmRunner.indexOf('run_frontend_build_auto', buildCase) < crmRunner.indexOf('ensure_playwright_chromium', buildCase),
    'Playwright must be installed after dependency alignment and build selection',
  )
})

test('shortcut launch detaches the local runtime and waits for a healthy manifest', () => {
  assert.match(launcher, /function Start-CrmGestorBackgroundUpdate/)
  assert.match(launcher, /Start-Process powershell\.exe/)
  assert.match(launcher, /iniciada em segundo plano/)
  assert.match(launcher, /\[int\]\$TimeoutSeconds = 420/)
  assert.match(launcher, /Wait-CrmPersonaCurrent/)
})
...
test('runtime policy reuses only a healthy build from the exact source snapshot', () => {
  ...
  const sourceOrigin = 'file:///tmp/origin'
  ...
  assert.deepEqual(decideRuntimeAction({
    ...current,
    manifest: { ...current.manifest, sourceOrigin: 'file:///tmp/origin-old' },
    sourceOrigin: 'file:///tmp/origin-new',
...
test('canonical CRM actions target updated main and ignore caller changes by default', () => {
...
test('a named CRM preview is shared across actions but applied to the current canonical commit', () => {
  ...
  assert.match(launcher, /A prévia ativa não deriva da revisão canônica solicitada/)
  assert.match(launcher, /deve estar no diretório privado autorizado/)
  assert.match(launcher, /rev-parse --show-toplevel/)
  assert.match(launcher, /sourceCommit = \$previewCommit/)
  assert.match(launcher, /sourceRoot = \$previewSourceRoot/)
  assert.match(launcher, /\$persistedCrmPreviewSelection = Get-Content/)
  assert.match(launcher, /\$crmCanonicalProjectRoot = 'C:\\CodexShared\\Projetos\\skincos'/)
  assert.match(launcher, /function Use-CrmLaunchSource/)
  assert.match(launcher, /if \(\$SelectedAction -like 'Crm\*'\)/)
})
...
test('a concurrent launcher reports an active runtime with a reusable status', () => {
  ...
  const helper = path.join(root, 'scripts', 'crm-local-persona-runtime.sh')
  const sourceFingerprint = 'commit:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'
  const command = `ROOT_DIR=${JSON.stringify(root)} CRM_RUNTIME_ROOT=${JSON.stringify(temp)} CRM_PERSONA=GESTOR CRM_SOURCE_FINGERPRINT=${JSON.stringify(sourceFingerprint)} CRM_RUNTIME_CONFIG_FINGERPRINT=test-config bash -c 'source ${JSON.stringify(helper)}; crm_persona_runtime_init; mkdir -p "$CRM_RUNTIME_LOCK_DIR"; crm_runtime_write_lock_owner "test"; crm_persona_runtime_acquire_lock; test "$?" = 2'`
  const result = spawnSync('bash', ['-lc', command], { encoding: 'utf8' })
  assert.equal(result.status, 0, result.stderr || result.stdout)
  assert.match(result.stdout.toLowerCase(), /runtime gestor/)
})
...
crm_persona_runtime_write_manifest ready`
  const result = spawnSync('bash', ['-c', body, 'bash', helper], {
    encoding: 'utf8',
    env: { ...process.env, ROOT_DIR: root, CRM_RUNTIME_ROOT: temp, CRM_PERSONA: 'GESTOR', CRM_TARGET_COMMIT: target, CRM_SOURCE_FINGERPRINT: `commit:${target}` },
  })
  assert.equal(result.status, 0, result.stderr || result.stdout)
