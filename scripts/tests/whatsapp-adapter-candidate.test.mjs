...
test('the declared portable closure runs in isolation and still refuses pre-cut creation', async () => {
  ...
  try {
    ...
    const inspected = inspectWhatsappAdapterCandidate({ candidate, evidence })
    ...
    const cli = spawnSync(process.execPath, [candidateScript, '--candidate', candidate, '--evidence', evidenceOutput.file], {
      cwd: candidate,
      encoding: 'utf8'
    })
    ...
    const portableResult = portableModule.inspectWhatsappAdapterCandidate({ candidate, evidence })
    assert.equal(portableResult.eligible, false)

    const rejectedInspect = spawnSync(process.execPath, [candidateScript, '--candidate', candidate, '--evidence', evidenceOutput.file, '--inspect'], {
      cwd: candidate,
      encoding: 'utf8'
    })
...
test('candidate gate rejects Evolution source and CRM state even when evidence is otherwise supplied', () => {
  ...
  try {
    ...
    assert.throws(
      () => inspectWhatsappAdapterCandidate({ candidate, evidence }),
  ...
  try {
    ...
    assert.throws(
      () => inspectWhatsappAdapterCandidate({ candidate: stateCandidate, evidence }),
      /must not contain prohibited path/
    )
  } finally {
  ...
  try {
    ...
    assert.throws(
      ...
      /must not contain unexpected directories/
    )
  } finally {
...
test('candidate gate verifies closure digest, source commit, tree and adapter source digests', () => {
  ...
  try {
    ...
    assert.throws(
      () => inspectWhatsappAdapterCandidate({ candidate, evidence: digestMismatch }),
    ...
    assert.throws(
      () => inspectWhatsappAdapterCandidate({ candidate, evidence: treeMismatch }),
...
test('candidate TAR is inspected without extraction and binds its raw archive digest', () => {
  ...
  try {
    ...
    assert.throws(
      () => inspectWhatsappAdapterCandidate({ candidate: archive, evidence: badEvidence }),
...
test('pre-cut status remains a hard creation and publisher block even if evidence is forged as proven', () => {
  ...
  try {
    ...
    assert.throws(
      () => assertWhatsappAdapterCandidateEligible({ candidate, evidence }),
