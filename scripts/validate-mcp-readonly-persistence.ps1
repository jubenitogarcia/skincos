...
$before = @(
    ...
    (Invoke-WslProbe 'readlink -f /opt/skincos/current/source'),
    (Invoke-WslProbe 'curl --silent --output /dev/null --write-out "orb_local=%{http_code}" http://127.0.0.1:5678/healthz'),
    (Invoke-WslProbe 'curl --silent --output /dev/null --write-out "orb_public=%{http_code}" https://orb.skincos.com.br/healthz'),
    (Invoke-WslProbe 'curl --silent --output /dev/null --write-out "public_mcp=%{http_code}" https://orb.skincos.com.br/mcp-server/http'),
...
@(
    "recovered_timestamp=$(Get-Date -Format o)",
    (Invoke-WslProbe 'systemctl is-active orb orb-proxy cloudflare-orb skincos-orb-mcp-readonly'),
    (Invoke-WslProbe 'readlink -f /proc/$(systemctl show --value -p MainPID skincos-orb-mcp-readonly.service)/cwd'),
    (Invoke-WslProbe 'curl --silent --output /dev/null --write-out "orb_local=%{http_code}" http://127.0.0.1:5678/healthz'),
    (Invoke-WslProbe 'curl --silent --output /dev/null --write-out "orb_public=%{http_code}" https://orb.skincos.com.br/healthz'),
    (Invoke-WslProbe 'curl --silent --output /dev/null --write-out "crm=%{http_code}" https://crm.skincos.com.br/api/health'),
    (Invoke-WslProbe 'curl --silent --output /dev/null --write-out "booking=%{http_code}" http://127.0.0.1:8765/healthz'),
    (Invoke-WslProbe 'curl --silent --output /dev/null --write-out "whatsapp=%{http_code}" https://wa.skincos.com.br/health'),
    (Invoke-WslProbe 'curl --silent --output /dev/null --write-out "public_mcp=%{http_code}" https://orb.skincos.com.br/mcp-server/http')
