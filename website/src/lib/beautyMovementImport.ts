...
/**
 * These values deliberately describe the preconfigured invitation outcome.
 * They are not generated by the cards and never mean a random prize.
 */
...
const OPTIONAL_HEADERS = ["email", "invite_status"] as const;
...
export function validateBeautyMovementImport(params: {
    ...
    if (!params.rewardCatalog || params.rewardCatalog.length === 0) {
        return {
            ok: false,
            delimiter: null,
            rows: [],
            sourceRowCount: 0,
            issues: [issue(null, null, "missing_reward_catalog")],
        };
    }
    ...
    for (let index = 1; index < parsed.rows.length; index += 1) {
        ...
        const rewardId = cleanText(value("reward_id"), 120);
        const reward = rewardById.get(rewardId);
        if (!/^[A-Za-z0-9][A-Za-z0-9._:-]{2,119}$/.test(rewardId)) {
            issues.push(issue(rowNumber, "reward_id", rewardId ? "invalid_reward_id" : "required_value_missing"));
        } else if (!reward) {
            issues.push(issue(rowNumber, "reward_id", "reward_not_found"));
        } else if (palette && reward.family !== palette) {
...
export async function prepareBeautyMovementImport(params: {
    ...
    const rewards = validateBeautyMovementRewardCatalog({
        catalog: params.rewardCatalog,
        procedureCatalog: params.procedureCatalog,
    });
    ...
    for (const row of validation.rows) {
        const reward = rewardById.get(row.rewardId);
        if (!reward) throw new Error("beauty_movement_reward_not_found");
        ...
        invites.push({
            ...
            benefitStatus: "evento_condicao_comercial",
            benefitText: reward.displayText,
            benefitValidity: reward.validity,
            benefitRules: reward.rules,
            termsVersion: reward.termsVersion,
    ...
    for (const invite of plan.invites) {
        statements.push(
            `INSERT INTO bm_invites (
                ...
                ${escapeSql(invite.palette)}, ${escapeSql(invite.rewardId)}, ${escapeSql(invite.velocityBenefit)},
