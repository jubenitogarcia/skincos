...
test("continuous experience reuses the real shell and keeps the inline finale flow", async () => {
    ...
    assert.match(experience, /onClick=\{startInitialDeal\}/);
    ...
    assert.ok(experience.indexOf("className={styles.heroDeckPrompt}") < experience.indexOf("className={styles.tableSurface}"));
    ...
    assert.match(styles, /\.heroDeckPrompt \{[\s\S]*display: block[\s\S]*background: transparent[\s\S]*cursor: pointer/);
    assert.match(styles, /\.heroDeckPrompt[\s\S]*font-family: var\(--font-brand-text\)/);
    ...
    assert.match(styles, /\.heroDeckPrompt/);
