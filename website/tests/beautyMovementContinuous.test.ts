...
test("continuous experience reuses the real shell and keeps the special-card finale flow", async () => {
    ...
    assert.match(experience, /renderSpecialCard\(false, "confirm", true, true\)/);
    ...
    assert.match(styles, /\.tableStage\[data-special-modal="open"\] \.specialCardStage \{[\s\S]*visibility: hidden;/);
    ...
    assert.doesNotMatch(experience, /operationalConsent|consentField|type="checkbox"/);
    assert.doesNotMatch(experience, /renderConfirmationAction|Garantir presente e confirmar presença/);
    assert.match(experience, /const commit = await onConfirm\?\.\(\{\s*email: null,\s*\}\)/);
    ...
    assert.match(experience, /if \(action === "confirm"\) confirmationActionRef\.current = node/);
    ...
    assert.match(styles, /\.specialCard\[data-special-state="revealed"\] \.specialCardBack \{[\s\S]*animation: specialCoveredFaceExit 980ms/);
    assert.match(styles, /\.specialCard\[data-special-state="revealed"\] \.specialCardFront \{[\s\S]*animation: specialRevealedFaceEnter 980ms/);
    assert.match(styles, /@keyframes cardFlipAndSettle \{[\s\S]*39% \{[\s\S]*rotateY\(90deg\)/);
    assert.match(styles, /@keyframes specialCardFlip \{[\s\S]*28% \{[\s\S]*rotateY\(90deg\)/);
    assert.match(styles, /@keyframes cardFlipToDeck \{[\s\S]*32% \{[\s\S]*rotateY\(90deg\)/);
