...
async function main() {
  ...
  const storageStatePath = path.join(ARTIFACT_DIR, 'storage-ponto.json')
  ...
  if (PERSISTENT) {
    ...
    context = await chromium.launchPersistentContext(path.join(ARTIFACT_DIR, 'profile-ponto'), {
      ...launchOpts,
      viewport,
    })
  } else {
  ...
  try {
    ...
    if (!authenticatedCookieHeader) {
      throw new Error('Authenticated Ponto session did not expose a revocable cookie')
    }
    await page.waitForTimeout(1500)
    ...
    if (JSON.stringify(visibleModuleKeys) !== JSON.stringify(['atendimento', 'ponto'])) {
      throw new Error('Ponto navigation does not expose exactly Atendimento and Ponto')
    }
