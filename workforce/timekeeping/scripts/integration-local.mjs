...
assert.equal(faceFailure.payload.code, 'FACE_DISABLED')
...
const deviceContext = await call('/api/ponto/device/context', { headers: { authorization: `Device ${createdDevice.payload.data.token}` } })
assert.equal(deviceContext.response.status, 200); assert.equal(deviceContext.payload.data.pinOnly, true)
const globalPinLock = await call('/api/ponto/device/punches', { method: 'POST', headers: { authorization: `Device ${secondDevice.payload.data.token}` }, body: { employeeCode: 'SYN001', pin: '9999', occurredAt: new Date(occurrenceBase + 330_000).toISOString(), requestId: `global-pin-lock-${integrationRun}` } })
assert.equal(globalPinLock.response.status, 429)
assert.equal(globalPinLock.payload.code, 'PIN_LOCKED')
const terminalFaceAttempt = await call('/api/ponto/device/punches', { method: 'POST', headers: { authorization: `Device ${createdDevice.payload.data.token}` }, body: { employeeCode: 'SYN001', descriptor, requestId: `device-face-${integrationRun}` } })
assert.equal(terminalFaceAttempt.response.status, 400); assert.equal(terminalFaceAttempt.payload.code, 'TERMINAL_PIN_REQUIRED')
const resetPin = await call('/api/ponto/pin/configure', { method: 'POST', actor: admin, body: { employeeId: 'synthetic-employee-001', pin: '1234' } })
assert.equal(resetPin.response.status, 200)
const devicePunch = await call('/api/ponto/device/punches', { method: 'POST', headers: { authorization: `Device ${createdDevice.payload.data.token}` }, body: { employeeCode: 'SYN001', pin: '1234', occurredAt: new Date(occurrenceBase + 360_000).toISOString(), requestId: `device-${integrationRun}` } })
assert.equal(devicePunch.response.status, 201)
assert.equal(devicePunch.payload.data.source, 'PIN')
